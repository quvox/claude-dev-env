...
## 10. Chrome/VNC コンテナによるリモート操作

`claude-dev start` を実行すると、Chrome/VNC 共有コンテナ（`claude-dev-chrome`）が自動的に起動する。ローカル PC のブラウザから noVNC 経由で Google Chrome を操作できる。
...
ローカル PC のブラウザ
    ...
    ▼
┌─ Chrome/VNC 共有コンテナ (claude-dev-chrome) ─┐
│  Xvnc :99    (TigerVNC, X + VNC 統合)        │
│  websockify  (noVNC, port 6080 → 5900)       │
│  openbox     (ウィンドウマネージャ)             │
...
### Chrome/VNC コンテナのライフサイクル

- `claude-dev start` 時に未起動なら自動起動される
- 全 Claude コンテナが停止すると Chrome コンテナも自動停止する
- 全プロジェクトで 1 つの Chrome コンテナを共有する
