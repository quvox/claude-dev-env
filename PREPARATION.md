...
## 10. コンテナ内 Chrome のリモート操作（`--chrome` オプション）

Claude Code コンテナ内で Chrome/Chromium の GUI を操作する必要がある場合（OAuth 認証、Web テスト等）、`--chrome` オプションで仮想ディスプレイ + VNC 環境を有効にできる。
...
┌─ Claude コンテナ ──────────────────────────┐
│  Xvfb :99     (仮想ディスプレイ)            │
│  x11vnc       (VNC サーバー, port 5900)     │
│  websockify   (noVNC, port 6080)           │
│  openbox      (ウィンドウマネージャ)          │
│                                             │
│  chromium-browser  ← tmux 内から起動        │
│  Claude Code       ← 通常通り使える         │
└─────────────────────────────────────────────┘
```
...
# → noVNC URL が表示される（例: http://localhost:6080/vnc.html）
```
...
ローカル PC のブラウザで表示された noVNC URL にアクセスすると、コンテナ内の Chrome を操作できる。

### 複数プロジェクトでの同時利用

noVNC ポートは 6080 から自動で空きポートが割り当てられる。

```bash
# プロジェクト A: --chrome → port 6080
cd ~/repos/project-a
claude-dev start --chrome

# プロジェクト B: --chrome → port 6081
cd ~/repos/project-b
claude-dev start --chrome
...
`--chrome` なしで起動すれば、VNC 関連のプロセスは一切起動しない（従来と同じ動作）。

```bash
claude-dev start   # VNC なし（軽量）
```
...
```bash
SAMBA_SHARE_DIR=/home/devuser/repos    # 実際のパスに合わせる
SAMBA_USER=yourname                     # デフォルトから変更
SAMBA_PASSWORD=your-secure-password     # デフォルトから変更
```

```bash
# 一括セットアップ（ビルド + Samba 起動 + PATH 登録）
make setup
...
正常に起動したら `Ctrl-B D` で tmux から切断し、以下で後片付け:
...
- [ ] `claude-dev services status` で Samba が `✅` と表示される
- [ ] `make login` で OAuth 認証が完了している
- [ ] `claude-dev start` でコンテナが起動し tmux に接続できる
- [ ] ホストの ufw が有効で、必要なポートのみ開放されている
- [ ] `claude-dev start --chrome` で noVNC URL が表示される
- [ ] noVNC にブラウザでアクセスし、コンテナ内で `chromium-browser` が起動できる
