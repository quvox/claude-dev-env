...
`claude-dev` は「**プロジェクトのディレクトリごとに、隔離された Claude Code 環境（コンテナ）を 1 つ立て、その中で作業する**」ツール。要点は 4 つだけ。

### 1. コンテナは「ディレクトリ単位」

- **`claude-dev start` = そのディレクトリ専用のコンテナを起動する**コマンド。**起動したディレクトリにコンテナが紐づく**ので、別のディレクトリで `claude-dev start` すれば**別のコンテナ**が立つ（＝案件ごとに独立・同時並行できる）。
- 起動したディレクトリ**全体がコンテナ内に丸ごとマウント**され、コンテナの中では **`/workspace`** として見える（ホストでの編集は即コンテナに反映、その逆も同様）。
...
コンテナに入った（tmux に接続された）ら、Claude の動かし方は 2 つ。**どちらも中身は Claude Code**で、違いは「作業全体を監督する司令塔（オーケストレーター）を挟むかどうか」だけ。

- **① 素の Claude Code**：tmux 内で **`claude`** を実行 → 承認待ちなしの Claude Code がそのまま起動する（1 エージェントで対話しながら作業）。
- **② AI オーケストレーター**：ブレインストーミング（検討）→ 実行計画 → 複数 worker への並列委譲 → 相互レビュー、までを**自律で回す司令塔**。人間の関与を「検討」と「例外対応（介入）」だけに絞れる（設計は [docs/06](docs/06_orchestration.md)）。起動は 2 通り：
  - **一発で**：**`claude-dev orchestrate`** — コンテナが未起動なら**自動で起動してから**、tmux 内で AI オーケストレーターを立ち上げて接続する。ゴールを渡すこともできる：`claude-dev orchestrate "ユーザー認証を実装"`。
  - **手動で**：`claude-dev start` で入った後、tmux 内で **`claude-orchestrator`** を実行しても同じく起動する。

> つまり「素の Claude Code が欲しい → `claude`」「全体を任せて自律で回したい → `claude-dev orchestrate`（または tmux 内で `claude-orchestrator`）」。

### 3. 抜け方（コンテナは裏で動き続ける）

tmux の prefix は **`Ctrl-_`**（Ctrl とアンダースコア）。

- **一時的に離れる（デタッチ）**：`Ctrl-_` → `d`。tmux セッションは生きたまま、作業は継続する。
...
- **Docker を使う開発**：`docker compose up` などをコンテナ内から実行できる（bind mount を多用する「Docker 中心」の開発は後述の **VM モード**が最適）。
- **実ブラウザでの E2E**：noVNC で画面を見ながら、Claude Code に chrome-devtools MCP で Chrome を操作させ、Web アプリの動作を確認できる。
- **VM モード（Linux のみ）**：さらに仮想度の高い環境。**コンテナの中にゲスト VM を立て、その VM の中でネイティブ Docker を動かす**ことで、bind mount・compose・privileged がそのまま使える（後述「VM モード」）。
