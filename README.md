...
│
└── claude-dev-auth (volume) OAuth 認証情報（読み取り専用でコンテナにマウント）
```
...
# 2. セットアップ（初回のみ。ビルド + Samba 起動 + PATH 登録）
make setup
...
> `make setup` は .env 作成、Docker ネットワーク/ボリューム作成、全イメージビルド、Samba 起動、CLI の PATH 登録を一括で行う。個別に実行する場合は `make help` を参照。
...
3. **認証情報の保護** — 読み取り専用ボリュームから起動時にコピー。ファイアウォールで窃取先をブロック
4. **Docker ソケット非共有** — コンテナ脱獄を防止
5. **ブラックリスト FW** — ペーストサイト、Webhook、メタデータエンドポイント、SMTP、外部 SSH をブロック
6. **非 root 実行** — devuser で実行。UID/GID はホストに自動追従
...
│   ├── Dockerfile.samba               Samba ファイル共有
│   └── samba/
│       ├── smb.conf                   Samba 設定
│       └── entrypoint.sh             Samba 起動スクリプト
├── scripts/
