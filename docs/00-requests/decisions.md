...
keywords: [決定台帳, 隔離方針, docker-proxy, オーケストレーター, VMモード, 委任, ClaudeCodeバージョン, CodexCLI, Codexサンドボックス]
source: null
---
...
| D-27 | Codex CLI の同梱・バージョン方針・認証共有方式 | ①**同梱**: 配布 2 イメージ（`claude-cli`/`claude-vnc`）に OpenAI Codex CLI（npm パッケージ `@openai/codex`）を導入する。導入 `RUN` は Claude Code と同じ**配布ステージの終端レイヤー**に置く。②**バージョン**: CI の prepare ジョブで npm registry の最新版を具体バージョン（例 `0.146.0`）へ解決し、build-arg でピン留めしてインストールする。`@latest` の直書きはしない。`workflow_dispatch` の入力で特定バージョンを手動指定して焼き直せるようにする。③**認証**: `codex login --device-auth`（ヘッドレス前提のデバイス認証）を採り、`$CODEX_HOME/auth.json`（既定 `~/.codex/auth.json`）**のみ**を既存 `claude-dev-auth` ボリュームの `codex/` サブディレクトリ経由でコンテナ間・ホスト間共有する。共有方式は D-3 と同じ「起動時コピー＋30 秒ごとのバックグラウンド書き戻し」。`config.toml`・セッション/履歴はコンテナ固有に保つ。④**ログイン導線**: ホスト CLI に独立サブコマンド `claude-dev login-codex` を追加する（既存 `login` は claude 専用のまま。`logout` は共有ボリュームを空にする現状挙動を維持し、codex 認証も同時に消える）。⑤**スコープ**: 本決定は「開発者がコンテナ内で codex を使える状態」までであり、オーケストレーターの worker/レビューアーとして codex を常用するかは D-22 のまま未決。⑥**サンドボックス方針**: コンテナ内の codex は**既定では自前のサンドボックスを使わず**、**読み取り専用を明示要求された場合に備えて landlock バックエンドを有効化しておく**。`$CODEX_HOME/config.toml` に次の 3 鍵を既定として置く——`sandbox_mode = "danger-full-access"`、`approval_policy = "never"`、`[features] use_legacy_landlock = true`。`config.toml` が存在しない場合は 3 鍵を生成し、既に存在する場合は**利用者が書いた鍵と値を一切書き換えず、上記のうち書かれていない鍵だけを追記する**（不足既定の補完。冪等）。codex のサンドボックスを動かすためにコンテナ側の seccomp/AppArmor プロファイルを緩める案（`docker run --security-opt`）は採らない。landlock でも動かない `workspace-write`（＝書き込みを伴う自動化・QA 用途）は `danger-full-access` で走らせてよい | codex も claude 同様に更新が速く、`@latest` を直書きすると文字列が変化せずレイヤーキャッシュが永久ヒットして**中身だけ凍結**する（D-26 で実際に発生した事象）。内容由来キーを終端レイヤーに置く方針をそのまま横展開すれば、鮮度と `docker pull` の増分性を両立できる。npm パッケージは platform 別バイナリを optionalDependencies で取得し linux-arm64 も提供されるため、マルチアーキ配布（D-11）と両立する。`auth.json` は保存方式の既定が keyring ではなくファイル・パーミッション 0600・その場書き換えで、トークンリフレッシュにより内容が変わるため書き戻し同期が必要。認証の器を既存ボリュームへ相乗りさせるのは、インフラ作成・`logout`・`reset` の分岐を増やさないため。ログインは対象ごとに独立コマンドとし、既存 `login` の挙動を変えずに片方だけログインし直せるようにする。⑥について: codex 0.146.0 の Linux サンドボックスは bubblewrap（`/usr/bin/bwrap`）実装で、ユーザー名前空間の作成とマウント伝播の変更を必要とする。claude コンテナは Docker 既定 seccomp と `docker-default` AppArmor の下で動くため、(a) 既定 seccomp が `CLONE_NEWUSER` を拒否し `unshare -U` が EPERM になる、(b) seccomp を外しても AppArmor が `mount --make-rslave /` を拒否する——の2段で bwrap が起動できない（2026-07-31 に配布イメージで実測。`--security-opt apparmor=unconfined` 単独では解決せず、seccomp と AppArmor の両方を外して初めて bwrap が動いた）。既定の `sandbox_mode`（`read-only` / `workspace-write`）のままでは codex が起こすシェルコマンドが例外なく `exited 1` になるうえ、モデルが失敗を認識せず出力を捏造した観測がある（`echo MARKER_ONE` が失敗したのに `MARKER_ONE` を回答）。`codex doctor` もこの故障を検知しないため、無効化を既定にしない限り「使える状態」（⑤）に到達しない。隔離境界はコンテナ／ホスト間のみと決めており（D-1）、コンテナ内の二重サンドボックスは元から前提にしていない。claude 側も同じ理由で `permissions.defaultMode=bypassPermissions` を既定にしているので方針を揃える。逆にコンテナの confinement を外す案は、生ソケット非マウント（D-2）等で守っている境界そのものを弱めるため採らない。**landlock の追加について（2026-07-31 実測）**: `sandbox_mode` の既定を無効化しても、`--sandbox read-only` のようにサンドボックスを**明示要求する呼び出し**（コードレビューや文書監査を codex に依頼する経路）は config の既定を上書きするため bwrap 経路に戻り、読み取りコマンドまで全滅する。codex 0.146.0 には bubblewrap 以外に legacy landlock バックエンドがフィーチャフラグ（`features.use_legacy_landlock`）で残っており、landlock はユーザー名前空間を必要としないためコンテナの confinement を緩めずに動く。実測で「読み取り成功／`/tmp`・`/workspace` への書き込み拒否／名前解決不可」まで確認でき、`codex exec review --uncommitted` も動作した（フラグ無しの対照実行は bwrap エラーで無指摘）。よって読み取り専用の用途だけは内側の隔離を生かせるので有効化する。ただし `workspace-write` は landlock 経路でも書き込みが失敗するため実用にならず、書き込みを伴う自動化・QA は `danger-full-access` で走らせる（**隔離境界はコンテナ／ホスト間のみ（D-1）であり、コンテナ自体が隔離空間なのでこれ以上の隔離は不要**という一貫適用。2026-07-31 の人間判断）。既定に置くのは**不足鍵の補完**に留め、利用者が書いた鍵は書き換えない（プロジェクトごとに設定を変える余地を残す）。起動時に疎通確認を走らせて警告する案は、codex を使わない利用者にも毎起動コストがかかるため採らず、確認は E2E-6 に持たせる。なお `use_legacy_landlock` は 0.146.0 時点で deprecated（起動時に「will be removed soon」と警告）なので、撤去された場合は監査を添付方式（対象ファイルの内容をプロンプトに添付し codex にシェルを使わせない方式）へ退避する | 本変更, `.devcontainer/Dockerfile.claude`, `scripts/entrypoint-claude.sh`, `claude-dev`/`claude-dev-mac`, `.github/workflows/ghcr-images.yml` |
...
|---|---|---|---|---|
| D-19 | 03-impl 記述の粒度と実コードとの対応づけ | 各モジュールの 03-impl でどこまで詳細を書くか、コードのどの範囲を1ファイルに束ねるか | `02-design/system.md` のモジュール分割定義を逸脱しない（モジュール数は同表が正本。本欄に数を書き写さない）。同じコードを複数の03-implに重複させない。データ構造・IF・ロジックの意図を書き、行単位コードは書かない | 本移行タスク |
