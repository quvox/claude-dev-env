...
- 判断(**2026-08-18 に同梱外部バイナリまで射程を広げた。2026-08-22 にエージェントが使う MCP サーバーを対象へ加えた。見出しの文言は変えていない** — `DSN-*` の ID が識別子であり、見出しの改名は反映の座標を動かすため): イメージを4ステージに分ける——重い共通層の `base`、ブラウザ確認資産を積む
  ...
  **中身が入れ替わりうる同梱物は、すべて終端ステージの最終レイヤーにのみ置く。**
  ...
  1. **エージェント CLI**(Claude Code / Codex CLI): キャッシュキーには CI が解決した具体
     バージョンを使う。
  1'. **エージェントが使う MCP サーバー**(chrome-devtools MCP。`D0-dist-06` / `FR-env-12-13`):
     **エージェント CLI とまったく同じ扱いとする** — CI が解決した具体バージョンをキャッシュキー
     とし、両方の終端ステージへ等しく置く。導入層はエージェント CLI の導入層と同じ位置
     (同梱外部バイナリの設置層より前)に置く。
...
- 却下した案: `base` の途中で導入し日次タイムスタンプで無効化する — 新版が無い日も毎日失効する。
  ...
  **同梱外部バイナリをエージェント CLI の導入層より前に置く**(同日に却下した) —
  ...
  増分でなくなる。
  **MCP サーバーを実行時に取得したままにする**(2026-08-22 に却下した) — ブラウザ確認の成否が
  取得先の可用性とコンテナ内ファイアウォールに依存し続け、この設計判断が CLI について既に退けた
  「実行時の自動更新」と同じ不確定さをブラウザ確認だけが抱える。
  **MCP サーバーを同梱外部バイナリの設置層へ相乗りさせる**(同日に却下した) — 設置層のキャッシュ
  キーはファイルの内容だが、MCP サーバーの版は CI が解決する値であり、キーの由来が2種類混ざる。
...
- 影響する要件: FR-env-09, FR-env-11, FR-env-12, FR-env-13, NFR-perf-01, NFR-perf-02
