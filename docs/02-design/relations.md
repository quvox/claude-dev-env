...
- 順序性・冪等性・並行性:
  ...
  - 冪等: 2回目の `reset` も失敗にしない。**ただし共有ネットワークが1回目で消えていると、遊休判定は「問い合わせ失敗」になって安全側へ倒れ、残した資源が無いのに「完全な初期状態には戻っていません」と表示する**(終了コードは 0 のまま)。`MODULE-cli-reset` の「既知の制限」がこの事実を持つ。
