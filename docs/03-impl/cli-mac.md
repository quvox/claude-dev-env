...
- **処理の要点:**
  ...
  - **`stop`:** コンテナ削除後、当該コンテナ内から起動された compose コンテナ群（ラベル
    `com.docker.compose.project=<正規化NAME>`）と当該プロジェクトの compose デフォルトネットワーク
    （`<正規化NAME>_default`）を削除（`docker compose down` 相当。名前付きボリューム・共有
    `claude-dev-net`・docker-proxy は残す。cli.md 正本）→`stop_ssh_bridge "$NAME"`（そのプロジェクトの
...
- **処理の要点:** `detect_docker_sock` は `/var/run/docker.sock` →`$HOME/.docker/run/docker.sock`
  ...
  さらに Linux 版と同じく `COMPOSE_OPTS`（`NAME` を compose 互換名へ正規化した
  `COMPOSE_PROJECT_NAME`）を `docker run` に `-e` で付与し、compose 既定名 `workspace` の
  プロジェクト間衝突を防ぐ（全シェル・`docker exec` で有効。詳細は cli 参照）。
...
| Docker ソケット不在 | `detect_docker_sock` 空 | docker-proxy を起動せず `DOCKER_HOST` 未付与で継続（要件 core/7-1 の DooD 前提が成立しない場合の縮退） | core/7-1 |
