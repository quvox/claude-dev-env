...
| **遊休と判定したあとの docker-proxy の削除に失敗した** | `set -e` によりその場で非0終了する(`docker rm -f "$DOCKER_PROXY_CONTAINER"` に `\|\| true` が無い)。**失敗を握らないのはこの1件と本体コンテナの2つだけである** | 最後の手順なので、後続に残る片付けは無い。共有資源単位のロックは `trap` が解放する |
