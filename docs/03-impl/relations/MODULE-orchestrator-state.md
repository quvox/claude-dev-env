...
7. `ArchiveRun()`(**引数なし**。`orchestrator/state.go:353`)が片付けを行う。run ID は引数ではなく
   `state.json` の `State.RunID` から取り、空または state が無ければ
   `run-<UTC yyyymmdd-HHMMSS>` を生成する。**削除ではなく `os.Rename` で `history/<run_id>/` へ退避**し、
