...
- **デバイス渡しの条件**: `claude-dev start`（ホスト側の `docker run`）が、ホストに存在する場合のみ `/dev/kvm`・`/dev/vhost-net`・`/dev/net/tun` を `--device` でコンテナに渡す。存在しなければスキップされる。privileged ではなく、個別デバイスのみを渡す。
...
- **リスク評価**: `/dev/kvm` への直接アクセス自体は KVM の ioctl に限定され、コンテナ脱獄に直結はしない。ただしデバイス渡し一般はコンテナ隔離をわずかに緩める。`/dev/net/tun` はネットワーク面の自由度を上げる。信頼境界を厳しくしたい場合は、`claude-dev` の `start` でデバイスを渡さない（`KVM_OPTS` を無効化する）ことで仮想化機能をオフにできる。
