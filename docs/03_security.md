...
- **封じ込め**: 書き換え後パスを正規化して `..` 脱出を拒否し、既存祖先の symlink を解決して `PROJECT_DIR` 外を指すものを拒否する（プロジェクト内に置いた symlink 経由でホスト任意パスを掴む脱出を防ぐ）。
...
| `/workspace` 配下を bind するコンテナ作成 | **許可（書換）** | エージェントが既に持つ範囲。`PROJECT_DIR/<相対>` へ書き換え、`..`/symlink 脱出は封じ込めで拒否（既定有効・`CLAUDE_DEV_ALLOW_WORKSPACE_BINDS=0` で無効化可） |
