...
- `~/.config/gh`（読み取り専用、存在時のみ）＝ GitHub CLI `gh` の認証・設定（`hosts.yml` の oauth トークンを含む）。コンテナ内で `gh` を認証済みで使うために共有する。トークンは秘密情報だが、既に転送済みの SSH agent（git push を許す）と同等の信頼境界であり、RO 共有かつ blacklist FW 下でのみ露出する意図的なトレードオフ
...
| 検査項目 | ルール | 理由 |
|----------|--------|------|
| `HostConfig.Binds` / `Mounts`(type=bind) | **条件付き許可**: source が呼び出し元の `/workspace` 配下なら実ホストパスへ書き換えて許可、それ以外のホストパスは**拒否** | ホストファイルシステムへの無制限アクセスを防ぎつつ、プロジェクト配下（＝エージェントが既に全 RW を持つ範囲）に限り bind を許す（後述「/workspace 配下 bind の許可」） |
...
**`/workspace` 配下 bind の許可（プロジェクト限定の緩和）:** 生ソケット直マウントで最も危険なのは任意ホストパスの bind だが、**呼び出し元プロジェクトの `/workspace` 配下だけ**は bind を許してよい——そこはエージェントが既に `/workspace` として全 RW を持つ範囲であり、内側コンテナへ bind しても新たな権限は増えないため。ただし DooD ではコンテナが送る `/workspace/...` を解決するのは**ホストのデーモン**で、ホストに `/workspace` は無く実体は各プロジェクトの `PROJECT_DIR`（claude コンテナが `/workspace` にマウントしているホストパス）である。そこで proxy は次を行う（実装 [50_docker-proxy.md](impl/50_docker-proxy.md)）:

- **呼び出し元特定**: リクエストの送信元 IP から、proxy が Docker API（`GET /containers/json`）で当該 claude コンテナを引き当て、その `/workspace` マウントのホスト側 source（=`PROJECT_DIR`）を得る（結果はキャッシュ）。特定できなければ**従来どおり拒否**（安全側）。
- **パス書き換え**: source が `/workspace`（およびその配下）の bind/mount を `PROJECT_DIR/<相対>` に書き換えて中継。`/workspace` 外のホストパスは拒否。
- **封じ込め（字句的）**: 書き換え後パスを正規化（`filepath.Clean`）して `..` 脱出を拒否し、`PROJECT_DIR` 配下に限定する。**symlink の実体解決は行わない**——proxy コンテナはホスト FS を一切持たず（docker socket のみ。かつ proxy へは `docker exec` が許可されるためホスト FS をマウントするのは危険）、ホストパスの symlink を解決できないため。結果として、プロジェクト内に置いた symlink がホスト外を指す場合の脱出は防げない（残存リスク・後述）。
- **無効化**: 環境変数 `CLAUDE_DEV_ALLOW_WORKSPACE_BINDS=0`（`false`/`no`/`off` も可）で本緩和を切り、全ホスト bind 拒否（従来動作）へ戻せる（既定は有効）。

これは DooD で実ホスト fs に bind するため、VM モード（virtiofs の uid 制約で DB 等の chown が失敗する）と異なりデータディレクトリの chown も通る。VM モードとは併存し、用途で使い分ける。
...
| 攻撃 | 結果 | 理由 |
|------|------|------|
| `/workspace` 外のホストパスを bind するコンテナ作成 | **拒否** | proxy が `Binds`/`Mounts` を検査し、`/workspace` 配下以外のホストパスを拒否 |
| `/workspace` 配下を bind するコンテナ作成 | **許可（書換）** | エージェントが既に持つ範囲。`PROJECT_DIR/<相対>` へ書き換え、`..` 脱出は字句的封じ込めで拒否（既定有効・`CLAUDE_DEV_ALLOW_WORKSPACE_BINDS=0` で無効化可。symlink 脱出は残存リスク） |
...
- **`/workspace` 配下 bind の symlink 脱出**: `/workspace` 配下 bind 許可（§5）の封じ込めは字句的で、proxy はホスト FS を持たず symlink を解決できない。エージェントがプロジェクト内に「ホスト外を指す symlink」を作り、それを bind source に指定すると、ホスト daemon が実体（例 `/etc`）をマウントし得る。緩和が不要なら `CLAUDE_DEV_ALLOW_WORKSPACE_BINDS=0` で無効化する（全ホスト bind 拒否＝従来動作）。より強い隔離が必要なら VM モードを使う
...
- `claude-dev start` 時にホスト側で**プロジェクト専用の ssh-agent**（`~/.claude-dev/agents/<ディレクトリ名>.sock`）を起動し、そのプロジェクトで指定した鍵**だけ**を `ssh-add` して転送する。ホストの環境 agent（全鍵を保持しうる）は転送せず、コンテナから見える鍵をディレクトリ単位で最小化する（最小権限）。使う鍵は `<プロジェクト>/.claude-dev.yaml` の `ssh_keys` **のみ**で指定する（グローバル設定へのフォールバックはなく、未指定なら SSH 転送なし）
