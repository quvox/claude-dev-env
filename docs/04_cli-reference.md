...
claude-dev orchestrate --fresh                # 前回 run を退避して新規開始（plan/履歴は消さず history/ へ）
```

- ゴール引数は任意。省略するとブレインストーミングから開始する。
- 起動時、**未完了の plan が残っていれば自動でその続きから再開**する（実行中に中断した場合も、`終了`(abort) で終えたが未完了タスクが残る場合も）。**全タスクが完了している／plan が無い**場合だけブレインストーミングから新規開始する。
- **plan・状態・履歴を自動で削除しない**：新規開始や `--fresh` で現 run を片付ける際も、`plan.json`・`state.json` 等は `.orchestrator/history/<run_id>/` へ**退避（アーカイブ）**して参照可能に保つ。実ファイルを消すのは利用者が明示的に `rm` した時だけ。
- `--fresh` は「未完了 plan が残っていても新規開始したい」ときに使う（現 run を `history/` へ退避し、worktree と `orch/*` ブランチは撤去してブレインストーミングから開始）。「ブレインストーミングを飛ばしたい／別ゴールで最初からやり直したい」状態のリセットに使う。
