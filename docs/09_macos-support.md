...
- **鍵の選択（インタラクティブ＋config 永続化）**: 転送する鍵は `~/.config/claude-dev.yaml` の `ssh_keys` に保持する。
  ...
  - **選択のリセット**: `claude-dev ssh-keys reset` で config の選択を消去し、専用 agent の鍵をフラッシュ（`ssh-add -D`）**して専用 agent も停止**する。次回 `start` で再度対話選択。`claude-dev ssh-keys` は任意タイミングでの再選択。
...
- **TCP ブリッジ**: ホストで `socat TCP-LISTEN:127.0.0.1:<port>,fork,reuseaddr` ↔ `UNIX-CONNECT:~/.claude-dev/ssh-agent.sock` を常駐させ、コンテナ側 entrypoint が `socat UNIX-LISTEN:/tmp/ssh-agent.sock,fork,mode=600`（ユーザー所有・0600）↔ `TCP:host.docker.internal:<port>` を張って `SSH_AUTH_SOCK=/tmp/ssh-agent.sock` を各シェルに設定する。CLI は `docker run` に `-e CLAUDE_DEV_SSH_BRIDGE_PORT=<port>` を渡す。
