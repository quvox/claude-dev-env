---
summary: macOS（Docker Desktop）上で Claude Code 安全開発環境を動かすための設計文書。Linux 版 claude-dev の OS 依存箇所を洗い出し、macOS 版 CLI（claude-dev-mac）での解決方針（SSH agent は claude-dev 専用 agent＋socat TCP ブリッジで転送＝鍵は対話選択して config 保存・Docker ソケット検出・VM/KVM 非対応・ポート直結・Apple Silicon の arm64 ネイティブ対応＝Playwright Chromium／gcloud アーキ写像・install は sudo symlink）を定める。
...
### 1. SSH agent の転送（最重要。方式 D: claude-dev 専用 agent＋TCP ブリッジ）
...
- **鍵の選択（インタラクティブ＋config 永続化）**: 転送する鍵は `~/.config/claude-dev.yaml` の `ssh_keys` に保持する。
  - **未選択時**（config に `ssh_keys` セクションが無い）は `start` 時に `~/.ssh/id_*`（`.pub` 除く）を列挙して**対話選択**させ、選んだパスを config に書き込む。以後は再プロンプトせず config の鍵を使う。
  - **選択のリセット**: `claude-dev ssh-keys reset` で config の `ssh_keys` を消去し、専用 agent の鍵をフラッシュ（`ssh-add -D`）**して専用 agent とブリッジ socat を停止**し、`ssh-agent.pid`/`ssh-bridge.pid`/`ssh-bridge.port`/`ssh-agent.sock` を削除する。次回 `start` で再度対話選択。`claude-dev ssh-keys` は任意タイミングでの再選択。
- **専用 agent**: ホストに claude-dev 専用 ssh-agent（固定ソケット `~/.claude-dev/ssh-agent.sock`）を起動し、選択鍵を `ssh-add`（パスフレーズ付きは起動時に一度だけ対話入力）。利用者の個人 agent（Strongbox 等）とは完全に独立。
- **TCP ブリッジ**: ホストで `socat TCP-LISTEN:<port>,bind=127.0.0.1,fork,reuseaddr` ↔ `UNIX-CONNECT:~/.claude-dev/ssh-agent.sock` を常駐させ、コンテナ側 entrypoint が `socat UNIX-LISTEN:/tmp/ssh-agent.sock,fork,mode=600`（ユーザー所有・0600）↔ `TCP:host.docker.internal:<port>` を張って `SSH_AUTH_SOCK=/tmp/ssh-agent.sock` を各シェルに設定する。CLI は `docker run` に `-e CLAUDE_DEV_SSH_BRIDGE_PORT=<port>` を渡す。（listen を 127.0.0.1 に限定するのは socat の `bind=` オプション。`TCP-LISTEN:` の第1引数はポート番号のみ。）
- **ポート**: `.env` の `CLAUDE_DEV_SSH_BRIDGE_PORT` があればそれを使い、無ければ空き 127.0.0.1 ポートを確保して `~/.claude-dev/ssh-bridge.port` に記録・再利用する（全 claude コンテナで 1 つ共有）。
- **前提**: ホストに `socat`（`brew install socat`）。`host.docker.internal` は Docker Desktop が提供（要到達確認済み）。
- **露出範囲と寿命**: ブリッジは **127.0.0.1 のみ**に listen し、**claude コンテナが 1 つ以上稼働している間だけ**起動する（`stop` で全コンテナが無くなればブリッジを停止。専用 agent は鍵を保持したまま常駐＝TCP 露出なし。例外として `ssh-keys reset` 時のみ専用 agent も停止する）。露出するのは**選択した鍵だけ**（個人 agent 全体ではない）。
- **魔法ソケット方式は使用しない**（`/run/host-services/ssh-auth.sock` のマウントは廃止）。Linux 版の `$SSH_AUTH_SOCK` 直 bind mount も macOS 版には存在しない。
- `~/.ssh/known_hosts` / `~/.ssh/config`（`IdentityFile`/`IdentitiesOnly` 行除去）の RO マウントは従来どおり。
...
- SSH 鍵を使う場合: 秘密鍵ファイルが `~/.ssh/` にあること（初回 `start` で対話選択→`~/.config/claude-dev.yaml` に保存。§1）。ホストの個人 agent（Strongbox 等）には依存しない
