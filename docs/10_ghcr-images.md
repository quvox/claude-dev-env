...
- 同じ `YYYYMMDDHHmm` を **イメージのラベル**（`io.github.quvox.claude-dev.version`、標準キー `org.opencontainers.image.version` も同値）にも焼き込む。これにより `docker tag` でローカル名へ retag してタグ情報が失われても、`claude-dev start` がバージョンを表示できる（ローカルビルドは `local`。ラベルが無い旧イメージは `unknown`＋作成日時にフォールバック）。
