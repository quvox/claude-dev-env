...
- **ブラウザ操作**: Claude Code の組み込みブラウザツール（computer use）で Chrome を直接操作。MCP 不要
...
VNC ありコンテナでは、Claude Code の組み込みブラウザツール（computer use）でコンテナ内の Google Chrome を直接操作する。MCP サーバーは不要。
...
│  IBus-Mozc             ← 日本語入力                           │
│                                                               │
│  Google Chrome         ← Xvnc 上で動作                        │
│                                                               │
│  Claude Code           ← 組み込みブラウザツールで Chrome 操作   │
│  tmux                  ← 開発作業                             │
│                                                               │
...
| 操作方法 | MCP Chrome DevTools（socat 二段リレー） | Claude Code 組み込みブラウザツール |
...
1. `claude-dev start` でコンテナ起動。entrypoint が Xvnc → openbox → Chrome → noVNC を順に起動
2. Claude Code が組み込みブラウザツールで Chrome を直接操作（スクリーンショット、クリック、入力等）
3. ユーザーは noVNC (`http://localhost:<port>/vnc.html`) で操作をリアルタイムに確認
4. `--no-vnc` の場合は VNC/Chrome 関連のプロセスが起動しない
...
│ 8. ファイアウォール設定                           │
│ 9. VNC/Chrome 起動（VNC ありイメージの場合）       │
│ 10. CLAUDE.md にコンテナ環境情報を書き込み         │
│ 11. tmux セッション開始                          │
└──────────────────────────────────────────────────┘
```
...
```bash
claude-dev list              # 全セッションの noVNC URL + ポートブロックを表示
claude-dev ports [NAME]      # プロジェクト単位の詳細ポート一覧
claude-dev ssh-forward [NAME] # SSH ポートフォワードコマンドを生成（noVNC ポート含む）
```
...
│       + Claude Code + 開発ツール一式
│
├── Target: claude-dev-claude (FROM base)
│   └── VNC なしイメージ（そのまま）
│
└── Target: claude-dev-claude-vnc (FROM base)
    └── + Google Chrome + TigerVNC + noVNC + openbox + IBus-Mozc
```

- 1 つの Dockerfile で両方のイメージをビルド
- `docker build --target claude-dev-claude` → VNC なしイメージ
- `docker build --target claude-dev-claude-vnc` → VNC ありイメージ
- ベースレイヤーは Docker のキャッシュで共有されるため、ディスク効率が良い
- `make build-claude` で両方のターゲットをビルドする
