...
5. **AI オーケストレーター** — `orchestrator/`（Go）。プロジェクトごとに 1 体立て、ブレインストーミング（人間との検討）→ 実行（`claude -p` worker への並行委譲・品質ゲート・タスク単位の介入）を駆動する。イメージへ `claude-orchestrator` として同梱（→ [60_orchestrator.md](60_orchestrator.md)）。自己検証用のサンプルサブプロジェクトは `examples/orch-sample/`（→ [70_sample-project.md](70_sample-project.md) / [docs/07_self-verification.md](../07_self-verification.md)）。
...
| ボリューム | 共有: `claude-dev-auth` / `claude-dev-history` / `claude-dev-config` ／ コンテナごと: `claude-dev-chrome-<name>`（Chrome プロファイル）・`claude-dev-vm-<name>`（VM モード） |
