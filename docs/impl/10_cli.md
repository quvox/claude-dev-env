...
ゴールを省略するとブレインストーミング（検討）から開始する。`--fresh` は前回の実行状態を破棄してブレインストーミングから新規開始するフラグで、そのままバイナリへ受け渡す（再開/新規の判定は `claude-orchestrator` 側、[60_orchestrator.md](60_orchestrator.md) 参照）。worker のライブ出力は各 worker ウィンドウ（`orch-<CNAME>-main:w-<taskID>`）で直接確認する（ダッシュボードで番号キー〔`[1-9]`〕により当該ウィンドウへ `select-window`。`prefix+w` でも一覧・選択可。旧 `[d]`／`--workers-window`／Config B は廃止）。メインセッションの `dashboard` ウィンドウは `remain-on-exit off`（worker/brainstorming ウィンドウは `remain-on-exit on`）。正常完了時は全窓が閉じてセッションが自然消滅するが、中断/クラッシュ時は `remain-on-exit on` 窓が空き殻セッションを残すため、生存判定は `has-session` ではなく上記の**コントローラプロセス生存**で行う（06 §5.9）。**VM モード対応**: コントローラ起動コマンド前に `[ -f /etc/claude-dev/vm.env ] && . /etc/claude-dev/vm.env` を挟み、VM モード時はゲストの `DOCKER_HOST` を orchestrator（および worker）へ引き継ぐ（非対話起動は rc を読まないため。詳細 [80_vm-mode.md](80_vm-mode.md)）。
