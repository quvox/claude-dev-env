...
| `VOL_AUTH` / `VOL_HISTORY` / `VOL_CONFIG` | `claude-dev-auth` / `-history` / `-config` | **共有**ボリューム名（全コンテナ共通）。Chrome プロファイルは**共有しない**：コンテナごとに `claude-dev-chrome-<name>`（VM ボリューム `claude-dev-vm-<name>` と同方式）を割り当てる（`start` §8 参照）。共有すると複数コンテナ同時起動時に同一プロファイルへ多重書き込みし、SingletonLock 奪取・プロファイル破損・Chrome 異常終了を招くため |
...
| `find_available_novnc_port` | `NOVNC_BASE_PORT` から +100 の範囲で、既存コンテナが公開していない空きポートを返す（見つからなければ基準値）。**選定〜`docker run` は非アトミックなため、複数 `start` をほぼ同時に実行すると同一ポートを選ぶ競合があり得る**。これは `start` の `docker run` リトライ（下記 §9）で吸収する。 |
...
| `ensure_infrastructure` | ネットワークと共有 3 ボリューム（auth/history/config）を冪等に作成。**コンテナごとの Chrome プロファイルボリューム `claude-dev-chrome-<name>` は `docker run` が自動作成する**ためここでは作らない。 |
...
### `setup`
`.env` を生成（無ければ `.env.example` から）、ネットワーク・共有 3 ボリューム（auth/history/config）を作成、`IMG_CLAUDE`（base）・`IMG_CLAUDE_VNC`（vnc）・`IMG_DOCKER_PROXY` を順にビルド。最後に次手順を案内し、PATH 未登録なら symlink 作成コマンドを表示。
...
8. **マウント/オプション組み立て**:
   ...
   - `NOVNC_PORT_OPT`（VNC 時のみ）: 空き noVNC ポートを `find_available_novnc_port` で確保し `-p <port>:6080` + **コンテナごとの Chrome プロファイルボリューム `claude-dev-chrome-<name>`** を `~/.chrome-profile` にマウント（VM ボリュームと同じくコンテナ名で分離し、複数コンテナ同時起動時のプロファイル競合を防ぐ。`docker run` が未作成なら自動生成する）
...
9. **コンテナ起動**: 起動直前に **使用イメージ名とバージョン**（`image_version "$RUN_IMAGE"`）を表示する。VM モード（`USE_VM=1`）のときは `docker run` の前に「VM モードで起動する／通常より時間がかかる（初回は cloud image 取得＋provision で数分）」旨を表示する。`docker run -d` で `--cap-add NET_ADMIN`・`NET_RAW`（FW 用）、`--restart unless-stopped`、`/workspace` マウント、各ボリューム、`tmux.conf`/`CLAUDE.md` を RO マウント、上記オプション群、`NODE_OPTIONS=--max-old-space-size=4096`、`-t` を付与。**`docker run` はポート割り当て競合に備えてリトライする**：複数 `start` をほぼ同時に実行すると `find_available_novnc_port` の選定〜`-p` バインドが競合し `docker run` が「port is already allocated」等で失敗し得るため、失敗時は作成途中コンテナを `docker rm -f` で掃除し、エラーがポート競合でかつ VNC 有効なら `find_available_novnc_port` で**別ポートを取り直して再試行**する（最大 20 回。取り直したポートは再試行メッセージで表示）。ポート以外の失敗や上限到達時は `docker run` の stderr を表示して `exit 1`。他プロセスによるポート横取りにも同経路で耐える。
...
### `reset`
確認プロンプト後、全 Claude コンテナ・全 `fwd-*`・プロキシコンテナを削除、共有 3 ボリューム（auth/history/config）・**全 `claude-dev-chrome-*`（コンテナごとの Chrome プロファイル）**・ネットワーク・3 イメージを削除。
