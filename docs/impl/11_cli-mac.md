...
**パス/設定（成果物）**
- 専用 agent ソケット: `${HOME}/.claude-dev/ssh-agent.sock`。
- ブリッジ状態: `${HOME}/.claude-dev/ssh-bridge.pid`・`${HOME}/.claude-dev/ssh-bridge.port`。
- ブリッジポート: `.env` の `CLAUDE_DEV_SSH_BRIDGE_PORT` があればそれを使い、無ければ空き 127.0.0.1 ポートを確保して port ファイルに記録・再利用（全 claude コンテナで 1 つ共有）。
- 鍵設定: `~/.config/claude-dev.yaml` の `ssh_keys`（下記スキーマ）。

**ヘルパー（成果物）**
| 関数 | 責務 |
|------|------|
| `discover_ssh_keys` | `~/.ssh/id_*`（`.pub` 除く）を列挙 |
| `config_has_ssh_selection` | config に `ssh_keys` セクションがあるか（選択済み判定） |
| `load_config_ssh_keys` | config の `ssh_keys` を `SSH_KEY_LIST` に読み込む（既存 YAML 簡易パース流用、`~`→`$HOME` 展開） |
| `select_ssh_keys_interactive` | `discover_ssh_keys` を番号付きで提示し、カンマ区切り番号 / `a`=全部 / `n`=なし で選択→選択パスを config の `ssh_keys` に書き込む |
| `ensure_dedicated_agent` | `${HOME}/.claude-dev/ssh-agent.sock` が生存していなければ `ssh-agent -a <sock>` で起動。選択鍵のうち未登録のものを `SSH_AUTH_SOCK=<sock> ssh-add`（パスフレーズ付きは対話で一度だけ） |
| `ensure_ssh_bridge` | `socat` の有無を確認（無ければ `brew install socat` を案内して SSH 転送をスキップ）。ブリッジ未稼働ならポートを決定し `socat TCP-LISTEN:127.0.0.1:<port>,fork,reuseaddr UNIX-CONNECT:<sock>` をバックグラウンド起動、pid/port を記録。使用中の `<port>` を返す |
| `stop_ssh_bridge_if_idle` | 稼働 claude コンテナが 0 なら socat を停止（pid を kill、pid/port ファイル削除）。専用 agent は残す（鍵保持・TCP 露出なし） |
...
- `claude-dev ssh-keys reset`: config の `ssh_keys` セクションを削除し、専用 agent の鍵をフラッシュ（`SSH_AUTH_SOCK=<sock> ssh-add -D`）して agent を停止。次回 `start` で再度対話選択。
