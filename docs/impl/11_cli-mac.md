...
`claude-dev-mac` は `claude-dev`（[10_cli.md](10_cli.md)）の macOS 適応版であり、**以下に列挙する差分以外は Linux 版と同一の成果物仕様**に従う。定数・ヘルパー関数・サブコマンド体系（`setup`/`pull`/`login`/`logout`/`start`/`code`/`orchestrate`/`attach`/`stop`/`forward`/`unforward`/`ports`/`list`/`upgrade`/`firewall`/`reset`/`help`）とその挙動は 10_cli.md を正本とする。**GHCR 対応（`pull` サブコマンド・`CONTAINER_USER` によるコンテナユーザー解決）は両 OS 共通**で 10_cli.md／設計 [../10_ghcr-images.md](../10_ghcr-images.md) を正本とし、macOS 版も同一実装を持つ（macOS 固有差分ではない）。
