...
- **内部サービスポートの除外（重要）**: claude コンテナ自身の内部サービス（noVNC `6080` / VNC `5999` / Chrome `9222`）は転送しない（`EXCLUDE`、`CLAUDE_DEV_DOOD_PORTSYNC_EXCLUDE` で上書き）。ホスト側で**別コンテナが同番ポートを `0.0.0.0` 公開**していると、その転送先 `127.0.0.1:PORT` がコンテナ内で自前サービスの bind と競合し、特に **dood-portsync が VNC 起動より前に走ると noVNC(6080) を先取りして websockify の起動を失敗させる**ため（`local_listening` の自己除外は起動順序に依存し確実でないので、明示除外で防ぐ）。
- **重複回避／自己除外**: 転送済みポートを `/tmp/dood-portsync/forwarded` に記録。既に `127.0.0.1:PORT` がローカル待受中（自分の転送含む。`/dev/tcp` で判定）ならスキップ。
