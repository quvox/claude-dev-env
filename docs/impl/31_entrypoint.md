...
- `DOCKER_HOST` / `SSH_AUTH_SOCK` / `CLAUDE_DEV_SSH_BRIDGE_PORT`（`docker run -e` 由来。`CLAUDE_DEV_SSH_BRIDGE_PORT` は macOS 版の SSH TCP ブリッジ用ポート）。
...
2. **~/.ssh 整備**: ディレクトリの所有権を設定し `chmod 700`。`~/.ssh/config` は CLI 側で `IdentityFile`/`IdentitiesOnly`/`IdentityAgent` を除去した一時コピーが RO マウントされる前提（正本 [10_cli.md](10_cli.md)/[11_cli-mac.md](11_cli-mac.md)）。
3. **KVM デバイスのグループ権限**: `/dev/kvm` `/dev/vhost-net` が存在すれば、そのデバイスの GID に一致するグループをコンテナ内に用意（無ければ `kvm-host-<gid>` を作成）し、`$USERNAME` を追加する。
4. **SSH agent の用意（2 経路）と `SSH_AUTH_SOCK` 永続化**:
   - **(macOS 版の TCP ブリッジ)** `CLAUDE_DEV_SSH_BRIDGE_PORT` が渡され、かつ `socat` があれば、既存 `/tmp/ssh-agent.sock` を削除後 `su "$USERNAME" -c "nohup socat UNIX-LISTEN:/tmp/ssh-agent.sock,fork,mode=600 TCP:host.docker.internal:${CLAUDE_DEV_SSH_BRIDGE_PORT} >/dev/null 2>&1 &"` で **`$USERNAME` 権限のバックグラウンド起動**（ホストの claude-dev 専用 agent への橋渡し。設計 [../09_macos-support.md](../09_macos-support.md) §1 / CLI 側 [11_cli-mac.md](11_cli-mac.md) D1）。起動後 `/tmp/ssh-agent.sock` が現れるまで **最大 20 回×0.2 秒待機**する。これにより `/tmp/ssh-agent.sock` がユーザー所有（0600）で用意される。
   - **(Linux 版の直マウント)** `CLAUDE_DEV_SSH_BRIDGE_PORT` が無く、`/tmp/ssh-agent.sock` が既に存在する（`$SSH_AUTH_SOCK` を直 bind mount した Linux 版）場合はそれをそのまま使う。
   - いずれかで `/tmp/ssh-agent.sock` が用意できたら `export SSH_AUTH_SOCK=/tmp/ssh-agent.sock` を `/etc/zsh/zshrc` と `/etc/bash.bashrc` に追記（`su -l` でのリセット対策）。`socat` はイメージに同梱済み。
...
13. **VM モードの起動（実装済み・要イメージ再ビルド反映。正本 [80_vm-mode.md](80_vm-mode.md)）**: `CLAUDE_DEV_VM=1` のとき、まず **root のうちに** `vm-up.sh` が書き込む root 所有ディレクトリを `$USERNAME` 所有で用意する（`install -d -o $USERNAME`：docker ボリュームのマウント点 `$USER_HOME/.claude-dev-vm` と実行時ディレクトリ `/run/vm` はいずれも既定で `root:root` のため、これを直さないと後続の `su $USERNAME -c vm-up.sh` 内の `mkdir` が Permission denied で失敗する）。その後 `su "$USERNAME" -c /usr/local/bin/vm-up.sh` で `scripts/vm-up.sh` を起動し（virtiofsd＋QEMU 常駐＋ゲスト dockerd 同期待ち）、成功時のみ `DOCKER_HOST` env スニペット（`/etc/claude-dev/vm.env`）出力・両 rc への source フック追記・`/workspace/VM_DEV.md` 生成・バナー表示を行う。失敗時は `DOCKER_HOST` を設定せず（proxy 既定維持）継続する。（macOS 版では VM モードは CLI 側で拒否されるため `CLAUDE_DEV_VM` は立たず、本ステップは no-op。）
14. **DooD ポート転送の起動（非 VM かつ proxy 経由。正本 [30_scripts.md](30_scripts.md) / [../02_architecture.md](../02_architecture.md)）**: `CLAUDE_DEV_VM != 1` かつ `CLAUDE_DEV_DOOD_PORTSYNC != 0` かつ `DOCKER_HOST` が `docker-proxy` を指すとき、`su "$USERNAME" -c "DOCKER_HOST=… setsid /usr/local/bin/dood-portsync.sh --loop &"` でホスト公開ポートを `127.0.0.1` へ転送する常駐同期を起動する。DooD では公開ポートがホスト側に出るため、コンテナ内テストが叩く `127.0.0.1:PORT` を到達可能にする（VM モードの `vm-portsync` に相当）。
15. **CLAUDE.md への環境情報追記**: マーカー `<!-- claude-dev-auto-start -->`〜`<!-- claude-dev-auto-end -->` で囲んだ範囲を毎回削除→再生成（旧形式セクションも除去）。「注意事項」、VNC 時は「Web アプリの動作確認」、**`/dev/kvm` が存在する時（`--kvm` 起動時）は「KVM / 仮想化（重要）」**（KVM 加速の有効化方法・QEMU 起動例・GUI VM を `:99` に表示する方法・computer-use/scrot での操作・ネットワーク・ディスクイメージ配置の注意）、常に「Docker ネットワーク（重要）」（コンテナ名でのアクセス指示）を書き込む。`/workspace/CLAUDE.md` が無ければ作成。これらはすべてマーカー範囲内に書かれるため、次回起動時に範囲ごと削除・再生成され、KVM の有無の変化（`--kvm` の付け外し）にも追従する。
...
17. **VNC / Chrome 起動（VNC 時のみ）**: `/tmp/start-user-desktop.sh` を生成し、ユーザー権限でバックグラウンド起動。内容は順に:
    ...
    - Chrome プロファイルの残存ロック（`SingletonLock` 等）を削除後、`claude-dev-chrome ... --remote-debugging-port=9222 --user-data-dir=~/.chrome-profile` を起動（`claude-dev-chrome` はアーキ別ランチャー: amd64=Google Chrome / arm64=Playwright Chromium。[40_devcontainer.md](40_devcontainer.md)）
    - システム D-Bus デーモン起動・GTK immodules キャッシュ更新も事前に実施
18. **tmux セッション開始**: ユーザー権限で `cd /workspace && tmux -f ~/.tmux.conf new-session -d -s main 'exec zsh -l'`。
19. `✅ Ready ...` を表示し、`exec tail -f /dev/null` で常駐。
