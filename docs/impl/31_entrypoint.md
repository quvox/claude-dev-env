...
13. **CLAUDE.md への環境情報追記**: マーカー `<!-- claude-dev-auto-start -->`〜`<!-- claude-dev-auto-end -->` で囲んだ範囲を毎回削除→再生成（旧形式セクションも除去）。「注意事項」、VNC 時は「Web アプリの動作確認」、常に「Docker ネットワーク（重要）」（コンテナ名・コンテナ名でのアクセス指示）を書き込む。`/workspace/CLAUDE.md` が無ければ作成。
14. **MCP 設定（VNC 時のみ）**:
