...
4. **SSH agent の用意（2 経路）と `SSH_AUTH_SOCK` 永続化**:
   - **(macOS 版の TCP ブリッジ)** `CLAUDE_DEV_SSH_BRIDGE_PORT` が渡されている場合、`$USERNAME` 所有で `socat UNIX-LISTEN:/tmp/ssh-agent.sock,fork,mode=600 TCP:host.docker.internal:${CLAUDE_DEV_SSH_BRIDGE_PORT}` をバックグラウンド起動する（ホストの claude-dev 専用 agent への橋渡し。設計 [../09_macos-support.md](../09_macos-support.md) §1 / CLI 側 [11_cli-mac.md](11_cli-mac.md) D1）。これにより `/tmp/ssh-agent.sock` がユーザー所有で用意される。
