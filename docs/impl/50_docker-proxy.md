...
3. **書き換え＋封じ込め** `containWorkspacePath(projectDir, containerSrc)`:
   ...
   - 既存する最深祖先を `filepath.EvalSymlinks` し、解決結果が（同様に EvalSymlinks した）`projectDir` 配下でなければ（symlink 脱出）**拒否**。
   - OK なら書き換え後ホストパスを返す。
...
- `/workspace` 配下 bind（`containWorkspacePath` / ボディ書換）: `/workspace/…`→`PROJECT_DIR/…` へ書換（`Binds` の opts 保持・`Mounts` の他フィールド保持）、`/workspace` 外のホストパス拒否、`..` 脱出拒否、symlink 脱出拒否、機能無効時は全ホスト bind 拒否。`resolveProjectDir` はテスト用に依存注入できる形にする（実 Docker API に触れず単体テスト可能にする）。
