...
- **ブレインストーミング（dashboard を先頭に保つ）**：コントローラは `orch-<CNAME>-main:brainstorming` ウィンドウを **`Run`（`new-window -d`）で起こすだけで `select-window` しない**（自分の pane では起動しない）。**必ず `dashboard` ウィンドウを先頭表示**に保ち（`SwitchTo(DashboardWindow)`）、そこへ `printBrainstormingHome(goal)` で**完結した案内画面**（goal・brainstorming 窓への移動方法＝`Ctrl-b w`／`Ctrl-b 数字`・`/exit` 後の遷移）を描画する。人間は自分で brainstorming 窓へ移動して対話する（自動で飛ばさない＝観測された「brainstorming 窓が先に出る／dashboard が描画途中に見える」不具合の対策）。`Handoff.WaitConsume`（`control.json` ポーリング＋`until`＝brainstorming ウィンドウ消滅/`pane_dead`。**毎ポーリングでの `select-window` は行わない**）で人間の `/exit` を待ち、戻ったら `dashboard` ウィンドウへ戻してメニュー（§handoff/§制御フロー step2）や実行遷移を行う。
...
- ブレインストーミングのウィンドウ化（`controller.go`）：`runBrainstorming` は tmux 有り時 `runBrainstormingSession`（brainstorming 窓を `Sessions.Run`〔`new-window -d`。`select-window` しない〕で起こし → `SwitchTo(DashboardWindow)` で dashboard を先頭に保ち → `printBrainstormingHome(goal)` で案内画面を描画 → `WaitConsume(until=!Has||PaneDead)` で `/exit` を待つ → 戻ったら `dashboard` へ）。**dashboard を先頭表示に保ち brainstorming 窓へ自動切替しない**（`LaunchInteractive`＝select-window する版は介入経路〔worker 窓〕でのみ使用）。実行/終了遷移時に `closeBrainstormingSession`。tmux 無し（headless/テスト）は従来の `RunInteractive` 前景フォールバック（`RunInteractive` は当該フォールバック専用として残す）。
