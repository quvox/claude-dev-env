---
summary: AI オーケストレーター（Go 製コントローラ）の実装仕様。tmux 常駐方式（コントローラは orch-<CNAME>-main セッション内で常駐）と独立セッション（壁打ち/worker/介入）・セッション管理と復旧・状態ストア・worker 並行ディスパッチ・品質ゲート・介入・判断基準・Slack 通知・ビルド配置を定める。設計の意図は docs/06_orchestration.md を参照。
...
├── mode.go                        モード切替（対話 claude の exec / WallbounceArgs での handoff_note 前置）。新アーキでは exec 先が該当 tmux セッション
├── session.go（新規・新アーキ）    tmux セッション管理（生成/終了/存在確認/再構築/mouse off/worker 選択切替/対話 claude のセッション内投入）
...
> **アーキテクチャ改訂（本改訂・実装は Phase③。tmux 常駐方式）**: コントローラは **`orch-<CNAME>-main` tmux セッション内で常駐**し（「常駐の器」は tmux サーバ＝クライアント全終了でもセッション保持）、壁打ち・各 worker・介入を**独立した tmux セッション**として起こして制御する（06 §4.1/§4.2/§5.3/§5.9）。対話（壁打ち/介入）は自分の pane ではなく該当セッション内で起動し、`control.json` のポーリングで `/exit` を検知する（自前景をブロックしない）。旧「run loop が端末前景を所有しモードで差し替える単一ウィンドウ方式」は廃止。詳細は下記「### 独立セッション方式（新アーキ）」。**現行のコード実装は tmux 常駐方式（実装済み）**。以降の本文で「`RunInteractive` で前景に exec」等と書かれた箇所は、**tmux が無い（headless/テスト）場合のフォールバック挙動**であり、実 tmux 環境では対話 claude は該当セッション内へ投入される（§実装状況が最新の成果物像）。

| モジュール | 責務 |
|---|---|
| controller | 状態機械（wallbounce/executing/done）と run loop の駆動。介入は executing 内のタスク単位イベントとして処理。**（新アーキ）`orch-<CNAME>-main` セッション内で常駐し、壁打ち/worker/介入の tmux セッション群を生成・終了・復旧する** |
| state | `/workspace/.orchestrator/` 以下のファイル群の読み書き。監査ログ追記 |
| mode | 壁打ち/介入＝対話 `claude` を子プロセスで exec。**（新アーキ）exec 先は該当 tmux セッション（wallbounce / w-<taskID>）内** |
| session（新規・新アーキ） | tmux セッション（`orch-<project>-main/-wallbounce/-w-<taskID>`）の生成・終了・存在確認・再構築・`mouse off`・worker 選択切替 |
| handoff | 対話 claude（壁打ち脳）からの遷移要求を `control.json` 経由で受け取る |
| worker | タスクを `claude -p` で worktree 上に実行し、構造化結果を回収。**（実装済み構成要素）各 worker に専用セッションを起こしログを `tail -F` でライブ表示（実行・結果回収は従来経路のまま＝セッションはビュー）。（新アーキ最終形）将来は `claude -p` 自体をセッション内へ投入** |
...
| dashboard | 実行モードの進捗・worker 状態・直近サマリを描画。**（新アーキ）メインセッションで worker セレクタ（番号〔数字キー〕で選択→当該セッションへ切替）を兼ねる。選択可能行にはタスク通番 `[i/n]` とは別列の選択番号 `‹k›` を明示し、キーヒントに `[1-9]worker画面へ` を出す（06 §5.3）** |
...
### 独立セッション方式（新アーキ。設計 06 §4.1/§4.2/§5.3/§5.9。実装済み）

コントローラを `orch-<CNAME>-main` セッション内で常駐させ、UI は独立 tmux セッションに分離する。以下が実装仕様。

- **tmux 常駐（setsid デーモンにはしない）**：`claude-orchestrator` は `claude-dev orchestrate` が起こす `orch-<CNAME>-main` セッションの pane で動き、その pane にダッシュボードを描画する。常駐性は tmux サーバが担保する——tmux クライアント（端末）が全終了しても切り離されたセッションは保持され、コントローラは走り続ける。よって setsid での完全デタッチ／`controller.log`／ダッシュボードの別プロセス化は**行わない**（完全デーモン化は端末を持てずダッシュボードを別プロセス＋ファイル IPC に分離する必要が生じ複雑化するため。06 §4.1 の判断）。**二重起動防止＝生存判定は `tmux has-session -t orch-<CNAME>-main`**：セッションが在れば既にコントローラが常駐中なので `claude-dev orchestrate` は attach するだけ、無ければ起こす。`main` は `remain-on-exit off`（コントローラ pane が死ねばセッションも消える＝has-session がそのまま生存信号）。`daemon.go` の pidfile ヘルパは補助的な二重起動ガードとして残す（主機構は has-session）。tmux サーバごと落ちた場合はコントローラも終了し、`.orchestrator/` の状態から resume（下記・復旧）。
- **セッション命名と管理（`session.go` 新規）**：`orch-<CNAME>-main`／`-wallbounce`／`-w-<taskID>`（`<CNAME>`＝正規化コンテナ名。既存 `COMPOSE_PROJECT_NAME` 相当の一意名）。生成は `tmux new-session -d -s <name>`（**コマンド直付けしない＝保持シェル**。理由は §下）、各セッションに `tmux set -t <name> mouse off`。終了は `tmux kill-session -t <name>`。存在確認は `tmux has-session`。
- **worker セッションは「保持シェル」方式（重要）**：`tmux new-session -d -s <name> "claude -p …"` のように**コマンド直付けで起こすと、`claude -p` 終了時にセッションも消えてしまい**、続く介入対話（同セッションで exec）が開けない。そこで worker セッションは**長寿命の保持シェル**（`tmux new-session -d -s <name>`＝既定シェルを保持）で作り、コントローラがその中へ**逐次コマンドを投入**する（`tmux respawn-pane -k -t <name> "<cmd>"` もしくは send-keys）：まず `claude -p …`（worker 実行）、要判断になれば同ペインへ対話 `claude`（介入）、回答後に再び `claude -p`（再ディスパッチ）。**セッションはコマンドの生死と独立に存続**し、コントローラが**タスク settle（done/failed/blocked）時にのみ `kill-session`** する。ライブ出力はそのセッションで直接見える（旧 `[d]` トグルを置換）。作業保全は従来どおり worktree。
- **セッションの生成タイミング（事前審査トリガーを含む）**：`orch-<CNAME>-w-<taskID>` は、そのタスクが**dispatch されるとき、または pre-dispatch トリガー（条件1＝後戻り不可の事前審査）で `waiting_human` として保留されるとき**に生成する（＝⏸ になったタスクには必ずセッションが在る）。これにより pre-dispatch 介入でも、セレクタから切り替える先のセッションが存在する（worker 実行前でも保持シェルに介入対話を出せる）。
- **メインセッションのダッシュボード＝セレクタ**：`orch-<CNAME>-main` で稼働 worker と要判断（⏸）を一覧描画し、**番号（数字キー）で worker を選択→`tmux switch-client -t orch-<CNAME>-w-<taskID>`** で当該セッションへ切替（`selectMenu` の描画・キー処理を流用）。`[p]`（一時停止）・`[q]`（中断）は従来意味。旧 `[d]`/`[i]` は廃止（`[d]`＝セッション直視、`[i]`＝セレクタ選択に置換）。
- **介入＝当該 worker セッション内で対話**：要判断（post-execution なら `claude -p` は終了済み、pre-dispatch なら未起動）になったら、人間がメインのセレクタで ⏸ を選ぶと、**その worker の保持シェルへ対話 `claude`（介入）を投入**する（当該 1 件のみを seed）。回答（`answer.md`＋タスク更新＋`control.json`）後 `/exit` で、同セッションへ `claude -p`（再ディスパッチ）を投入。**1 セッションに全件 seed する旧方式は廃止し、要判断は worker セッション単位で個別処理**する（旧 `resolveInterventions`／`ResolveArgs` の「全件 seed」契約は §モード切替/§制御フローの現行記述＝旧方式で、本アーキでは当該 1 件のみに変更）。
- **壁打ち**：コントローラは `orch-<CNAME>-wallbounce` セッション（保持シェル）を起こし、その中へ対話 `claude`（wallbounce.md 付き）を投入して attach クライアントをそのセッションへ `switch-client` する（自分の pane では起動しない）。コントローラ側は `Handoff.WaitConsume`（`control.json` ポーリング＋`until`＝wallbounce セッション消滅）で人間の `/exit` を待ち、戻ったらクライアントを `main` へ戻してメニュー（§handoff/§制御フロー step2）や実行遷移を行う。
- **単一コマンド復旧（`claude-dev orchestrate`）**：(1) `tmux has-session -t orch-<CNAME>-main` が真＝コントローラ常駐中 → `tmux attach -t orch-<CNAME>-main` するだけ。(2) 偽（tmux サーバ死・main 誤 kill 等）→ 新しい `orch-<CNAME>-main` を作りその中で `claude-orchestrator` を起こす（状態ストアから resume。Phase=executing なら実行継続）→ 起動後コントローラが `EnsureAll` で不足セッション（実行中タスクの `w-<taskID>`／壁打ち中なら `wallbounce`）を再構築 → その後 attach。人間はこれ一発で端末破壊・セッション誤終了から復旧する。コントローラも定期的に `EnsureAll` の点検・再構築を行う（誤って閉じられた worker セッションを実行中なら再作成）。
...
> **本節の「前景 exec／`[i]` 全件 seed」は tmux 無し時のフォールバック挙動の記述**。実 tmux 環境（現行の既定）では独立セッション方式（§「独立セッション方式（新アーキ）」）が動く＝対話 claude の投入先は該当 tmux セッション（wallbounce / w-<taskID> の保持シェル）、介入は ⏸ 選択で当該 1 件のみを対応（`ResolveArgsOne`）。最新の成果物像は §実装状況。
...
`dashboard.go` は 06 §5.2 ② の画面を描画する：**ヘッダに現在モード（`● 実行中`／`⏸ 一時停止`）**、goal、各タスクの `[i/n] worker X (claude): 状態ラベル 経過時間 (試行N)`（`waiting_human` は ⏸ 要判断ラベル）、直近サマリ、仮定カウント・**未解決の要判断件数と一覧（`intervention/open.json` の各 TaskID→タスク名。件数だけでなくどのタスクが待っているかを列挙。06 §5.5）**・実行中数、キーヒント（`[d]`/`[p]`/`[i]`/`[q]`）。worker の実行内容は `[d]` 詳細表示でログ末尾をライブ確認できる（別ウィンドウ＝旧 Config B は廃止し、`[d]` に一本化した）。`[i]` は未解決の要判断がある時のみ有効。人間向け表示はすべて日本語（06 §5.7）。**この画面は `orch-<CNAME>-main` セッションで描画される（実装済み）。数字キー `[1-9]` で ⏸ worker を選ぶとそのセッションで介入対話、実行中 worker はセッション直視へ切替（旧 `[d]` トグルを置換）。§「独立セッション方式（新アーキ）」／§実装状況**
...
**本改訂で独立セッション方式を実装（tmux 常駐方式。06 §4.1/§4.2/§5.3/§5.9・本書「独立セッション方式（新アーキ）」）**：

*実装・単体テスト済み（`go build`/`vet` 緑・gofmt 済み・`go test -race` 全緑）*：
- `session.go`：`SessionManager`（`orch-<CNAME>-main/-wallbounce/-w-<taskID>` の命名・`Ensure`〔保持シェル＋`mouse off`＋`remain-on-exit on`〕・`Run`〔respawn-pane 投入〕・`Kill`・`Has`・`SwitchTo`・`PaneDead`〔/exit 検知の副信号〕・`LaunchInteractive`〔保持シェルへ対話 claude を投入し `switch-client`〕・`ExpectedSessions`/`EnsureAll`〔復旧〕）と `tmuxAvailable`。`remain-on-exit on` により対話 claude の `/exit` 後もセッションが残り、`claude -p`→介入→再ディスパッチを同一セッションで駆動できる。テスト：命名正規化・セッション名・ExpectedSessions。
- `daemon.go`：pidfile（`writePidfile`/`readPidfile`/`processAlive`/`controllerAlive`）＝補助的な二重起動ガード（tmux 常駐方式の主機構は `tmux has-session`）。テスト：pidfile 往復・生存判定。
- 対話のセッション内投入（`mode.go`）：`wallbounceInstr`/`interveneInstr`（instruction 組立）・`IntervenePrompt`（1件の system/prompt ペア）・`WriteLaunchScript(key,sys,prompt)`（`.orchestrator/sessions/<key>.sh` に launcher を生成。VM env source・claude を PATH・`SLACK_BOT_TOKEN` strip・`cd` workspace・巨大 prompt は `.sys`/`.prompt` sidecar から `$(cat …)` で読む＝argv/quoting 肥大回避）・`shellSingleQuote`。テスト：`TestWriteLaunchScript`・`TestWriteLaunchScript_NoPromptOmitsPositional`・`TestShellSingleQuote`。
- 壁打ちのセッション化（`controller.go`）：`runWallbounce` は tmux 有り時 `runWallbounceSession`（wallbounce セッションへ `LaunchInteractive`→`WaitConsume(until=!Has||PaneDead、poll ごとに `SwitchTo` で attach 済みクライアントを引き込む)→`main` へ復帰）。実行/終了遷移時に `closeWallbounceSession`。tmux 無し（headless/テスト）は従来の `RunInteractive` 前景フォールバック（`RunInteractive` は当該フォールバック専用として残す）。
...
- worker セッション結線（`controller.go`）：dispatch と **pre-dispatch ⏸ 化**の両方で `openWorkerSession`（ログ tail のライブ表示。⏸ タスクに必ずセッションが在る不変条件）、settle で `closeWorkerSession`（`waiting_human` は残す）。executing ループは数秒に一度 `EnsureAll` 相当の点検で、実行中タスクの消えた worker セッションを `openWorkerSession` で再構築（誤 kill 復旧・06 §5.9）。`main.go` は `Sessions` を常に注入。
- worker セレクタ（`dashboard.go`）：数字キー→`selectableWorker`（id＋status）。**⏸（waiting_human）は `d.Resolve` へ流しコントローラが当該セッションで介入対話を起こす**、実行中等は `SwitchTo`（ビュー切替）のみ。テスト：`TestSelectableWorkerID`・`TestSelectableWorkerStatus`。**画面には選択番号を可視化**（06 §5.3）：`renderString` は選択可能行（実行中／レビュー中／修正中／⏸ 要判断）にだけ行頭選択番号（`‹k›`）を付す。**タスク通番 `[i/n]` とは別列の連番**（選択可能 worker のみを 1 から数える＝`selectableWorker` の n と一致）で、選択不可行は同幅の空白で桁揃え。選択可能 worker が 1 件以上あり `Sessions` 注入時のみキーヒント行に `[1-9]worker画面へ`。テスト：`TestRenderSelectorNumbers`・`TestRenderNoSelectorHintWhenNoneSelectable`・`TestRenderNoSelectorHintWithoutSessions`。
- handoff 監視：`Handoff.WaitConsume`（control.json をポーリング＋`until` で /exit 検知）。テスト：出現検知・until 終了。
- CLI（`claude-dev orchestrate`）：`--print-main-session` で本体からメインセッション名を得て、`has-session` 真→`attach` のみ／偽→新 `orch-<CNAME>-main` で `claude-orchestrator` 起動（resume）→`mouse off`→attach（→[10_cli.md](10_cli.md)）。従来の `tmux new-window -t main` 直起動は廃止。

*残（実機 E2E で最終確認）*：実 tmux＋実 claude の対話が必要なため自動テストでは検証不能。S1〜S5（[docs/07_self-verification.md](../07_self-verification.md)）を実機で確認し `docs/reviews/` に記録する（壁打ち→execute 遷移、⏸ 選択→当該セッションで介入→再ディスパッチ、端末 close→再 attach 復旧、worker セッション誤 kill→再構築）。
