...
├── session.go（新規・新アーキ）    tmux セッション管理（生成/終了/存在確認/再構築/mouse off/worker 選択切替）。デーモン化（setsid・pidfile）
...
> **アーキテクチャ改訂（本改訂・実装は Phase③）**: コントローラは**端末を占有せず常駐するデーモン**とし、各コンポーネント（メインループ/ダッシュボード・壁打ち・各 worker・介入）を**独立した tmux セッション**として起こして制御する（06 §4.1/§4.2/§5.3/§5.9）。旧「run loop が端末前景を所有しモードで差し替える単一ウィンドウ方式」は廃止。詳細は下記「### 独立セッション方式（新アーキ）」。**現行のコード実装は旧単一ウィンドウ方式**であり、本改訂の独立セッション方式は仕様確定・実装は Phase③（§実装状況）。

| モジュール | 責務 |
|---|---|
| controller | 状態機械（wallbounce/executing/done）と run loop の駆動。介入は executing 内のタスク単位イベントとして処理。**（新アーキ）端末非占有のデーモンとして tmux セッション群を生成・終了・復旧する** |
...
| dashboard | 実行モードの進捗・worker 状態・直近サマリを描画。**（新アーキ）メインセッションで worker セレクタ（↑↓/番号で選択→当該セッションへ切替）を兼ねる** |
...
### 独立セッション方式（新アーキ。設計 06 §4.1/§4.2/§5.3/§5.9。実装は Phase③）

コントローラを端末非占有の常駐デーモンにし、UI は独立 tmux セッションに分離する。以下が実装仕様。

- **デーモン化**：`claude-orchestrator` は起動時に自身をデタッチ（`setsid` 相当で新セッションリーダ化・制御端末を切り離し）してバックグラウンドで回り続ける。ログは `.orchestrator/controller.log` へ。人間向け tmux セッションの中では動かさない（そのセッションを閉じても道連れにならない）。二重起動防止は pidfile（`.orchestrator/controller.pid`）＋生存確認。
...
- **メインセッションのダッシュボード＝セレクタ**：`orch-<CNAME>-main` で稼働 worker と要判断（⏸）を一覧描画し、**↑↓/番号で worker を選択→`tmux switch-client -t orch-<CNAME>-w-<taskID>`** で当該セッションへ切替（`selectMenu` の描画・キー処理を流用）。`[p]`（一時停止）・`[q]`（中断）は従来意味。旧 `[d]`/`[i]` は廃止（`[d]`＝セッション直視、`[i]`＝セレクタ選択に置換）。
...
- **壁打ち**：`orch-<CNAME>-wallbounce` セッション内で対話 `claude` を exec。`/exit` 後にコントローラ（デーモン）が `control.json` を読み、メニュー（§handoff/§制御フロー step2）や実行遷移を行う。
- **単一コマンド復旧（`claude-dev orchestrate`）**：(1) pidfile でコントローラ生存を確認し、死んでいれば状態ストアから再開起動（Phase=executing なら resume）、(2) 期待するセッション（`main` 常設／実行中タスクの `w-<taskID>`／壁打ち中なら `wallbounce`）を `has-session` で点検し無ければ再構築、(3) `main` に `tmux attach`。人間はこれ一発で端末破壊・セッション誤終了から復旧する。コントローラも定期的に (2) の点検・再構築を行う（誤って閉じられた worker セッションを実行中なら再作成）。
...
> **本節は現行コード＝旧「単一ウィンドウ・前景所有」方式の記述**。本改訂の**独立セッション方式（§「独立セッション方式（新アーキ）」）では、対話 claude の exec 先が該当 tmux セッション（wallbounce / w-<taskID> の保持シェル）に変わり、`ResolveArgs` の「全件 seed」は当該 1 件のみに変わる**（実装は Phase③）。以下は移行前の実装として読むこと。
...
`docker exec -it -u <user> <name> tmux new-window -t main "claude-orchestrator …"` → `tmux attach` する。コンテナ起動は従来どおり `claude-dev start`。ゴール引数は任意（既定は壁打ちから開始、06 §5.1）。`--fresh` はそのままバイナリへ渡す（前回の実行状態を破棄して壁打ちから新規開始）。単一ウィンドウ構成のみ（worker 出力はダッシュボードの `[d]` で確認）。**（旧方式。新アーキ＝独立セッションでは `claude-dev orchestrate` は「コントローラ〔デーモン〕生存確認→無ければ状態から再開起動→必要セッション再構築→`orch-<CNAME>-main` へ attach」に変わる＝単一コマンド復旧。Phase③。10_cli.md／§「独立セッション方式（新アーキ）」）**
...
**本改訂で独立セッション方式へ移行中（Phase③。06 §4.1/§4.2/§5.3/§5.9・本書「独立セッション方式（新アーキ）」）**：

*構成要素＝実装・単体テスト済み（`go build`/`vet` 緑・gofmt 済み・`go test -race` 全緑）*：
- `session.go`：`SessionManager`（`orch-<CNAME>-main/-wallbounce/-w-<taskID>` の命名・`Ensure`〔保持シェル＋`mouse off`〕・`Run`〔respawn-pane 投入〕・`Kill`・`Has`・`SwitchTo`・`ExpectedSessions`/`EnsureAll`〔復旧〕）。テスト：命名正規化・セッション名・ExpectedSessions。
- `daemon.go`：pidfile（`writePidfile`/`readPidfile`/`processAlive`/`controllerAlive`）。テスト：pidfile 往復・生存判定。
- worker セッション結線（`controller.go`）：dispatch で `openWorkerSession`（ログ tail のライブ表示）、settle で `closeWorkerSession`（`waiting_human` は残す）。**`main.go` は `Sessions` を常に注入するため、本番（tmux 有り）ではこの worker セッション生成/kill と下記セレクタは実際に稼働する**（best-effort。tmux 無し・テストでは nil または no-op で既存挙動不変）。既存の worker 実行・結果回収の経路自体は不変。
- 介入の per-worker 構成要素：`mode.ResolveArgsOne`（1件シード）、`reconcileOne`/`resolveOne`（1件突合）。`resolveInterventions` を共通ロジックへ再構成。テスト：`TestResolveOne`。
- worker セレクタ（`dashboard.go`）：数字キー→`selectableWorkerID`→`SwitchTo`。テスト：`TestSelectableWorkerID`。
- handoff 監視：`Handoff.WaitConsume`（control.json をポーリング。デーモン watch-model 用）。テスト：出現検知・until 終了。

*最終結線＝未実装（要・実機 E2E。実 tmux＋実 claude の対話が必要で自動テストでは検証不能）*：
- コントローラのデーモン化（launcher〔`claude-dev`〕での `setsid` 分離＋`main.go` の pidfile 管理／`controller.log`）。
- 対話（壁打ち/介入）の **`RunInteractive`（プロセス終了ブロック）→ セッションへ launch＋`WaitConsume`（handoff 監視）への置換**、および selector で ⏸ 選択時に当該 worker セッションで介入対話を起動する結線。
- `claude-dev orchestrate` の「デーモン生存確認→無ければ状態から再開起動→`EnsureAll` でセッション再構築→`orch-<CNAME>-main` へ attach」への改修（→[10_cli.md](10_cli.md)）。
- selector で ⏸（waiting_human）を選んだ時に当該 worker セッションで介入対話を起動する結線（現状 selector は `SwitchTo` のみ）。および **pre-dispatch トリガー（条件1）で `waiting_human` 化したタスクの worker セッション生成**（現状 `openWorkerSession` は dispatch 時のみ。§「独立セッション方式」§セッション生成タイミングの不変条件はこの結線で満たす）。
- ※これらは動作中システムの中核（前景受け渡し・CLI）を書き換えるため、実機の対話 E2E で検証してから投入する。**現行コードの対話・実行の既定動作は引き続き旧「単一ウィンドウ・前景差し替え」方式**（`RunInteractive` ブロック・`[i]` バッチ介入・`claude-dev orchestrate` は new-window→attach）。その上に worker セッションのビュー表示とセレクタ切替が加わっている。
