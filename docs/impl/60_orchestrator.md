...
├── models.go                      モデル/effort 選択ポリシー表（工程別。ここだけ編集すれば全体反映）
...
## モデル選択（工程別の model / effort）

`models.go` が**唯一の編集ポイント**。各工程で使う `claude` の `--model` と `--effort` を「作業内容」で切り替える（コンフィグファイルにはしない＝設計/実装仕様と一緒に管理して変更容易にする方針。06 §モデル選択）。
...
- テスト：`models_test.go`（kind→profile 分類・ロール profile・effort 値の妥当性）、`mode_test.go`（launch script に `--model`/`--effort` が載る）。
