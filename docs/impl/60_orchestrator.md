...
- 壁打ちのセッション化（`controller.go`）：`runWallbounce` は tmux 有り時 `runWallbounceSession`（wallbounce ウィンドウへ `LaunchInteractive`→`WaitConsume(until=!Has||PaneDead)→`dashboard` へ復帰）。attach クライアントの引き込みは `LaunchInteractive` の初回 `select-window` のみ（毎ポーリングでの再切替はしない＝ユーザが dashboard 等へ自由に移動できるようにするため）。実行/終了遷移時に `closeWallbounceSession`。tmux 無し（headless/テスト）は従来の `RunInteractive` 前景フォールバック（`RunInteractive` は当該フォールバック専用として残す）。
