...
├── ORCHESTRATOR.md               プロジェクト固有の判断基準（60 §判断基準。worker/ブレインストーミング/介入に前置される）
├── GOAL.md                       ブレインストーミングを短縮するためのゴール記述（orchestrate のゴール引数に使う）
...
3. `--start-executing` で検証する場合のみ、`seed/plan.json` を作業コピーの `.orchestrator/plan.json` として配置する（通常のブレインストーミング検証では配置しない）。
...
- **本番同等**：`claude-dev start` 後、`claude-dev orchestrate --workspace workspace/orch-sample`（ゴールは `GOAL.md` を渡すかブレインストーミングで決める）。
- **オーケストレーター開発の高速ループ**（イメージ再ビルド不要）：
  ...
  - `--start-executing`（**本改訂で追加する検証用 affordance**）：`.orchestrator/plan.json` が存在し `Ready=true` のとき、ブレインストーミングを飛ばして直接 `executing` から開始する。**ready な seed plan が無ければ無効**（通常起動＝ブレインストーミングにフォールバック）。決定論的な非対話検証（S1〜S5）のために用意する。通常運用の既定は従来どおりブレインストーミング開始（06 §5.1）であり、この affordance は検証専用と明記する。
