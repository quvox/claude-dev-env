...
## 要件（なぜ必要か）
DooD＋socket proxy 構成では proxy がホスト bind mount を原則拒否し（`/workspace` 配下のみ実ホストパスへ書き換えて許可＝[../03_security.md](../03_security.md) §5）、DooD のパス不一致もあって **privileged や広範な bind を伴う**「Docker 中心開発」が DooD 経路では成立しにくい（08 §1）。VM モードは KVM のゲスト内でネイティブ Docker を動かし、virtiofs で `/workspace` を同一パス共有することで、**bind mount がライブ反映付きで成立**し（privileged 等も VM 境界内で使え）、かつ claude コンテナを privileged 化せず隔離を保つ。設計・意図は 08 を参照。
...
- **`orchestrate` の VM 対応**: `claude-dev orchestrate` は `claude-orchestrator`（非対話）を起動するため rc/vm.env を自動では読まない。起動コマンドの前に **`[ -f /etc/claude-dev/vm.env ] && . /etc/claude-dev/vm.env`** を挟み、ゲスト `DOCKER_HOST` を orchestrator（および `claudeChildEnv` 経由で worker）へ引き継ぐ。worker への VM モード周知は `--append-system-prompt` ではなく**発見導線2（orchestrator のプロンプト前置）**で行う（[60_orchestrator.md](60_orchestrator.md) の `VMModePreamble`＝`state.go`）。
...
2. **orchestrator のプロンプト前置** — worker/ブレインストーミングプロンプト先頭に `VM_DEV.md` へのポインタを前置する。実装は **[60_orchestrator.md](60_orchestrator.md) 側**（既存の `ORCHESTRATOR.md` 前置と同じ仕組みを流用。VM モード時のみ前置）。本書は連携先として指す。
...
- **health ファイル**: 各周回で `${CHOME}/.claude-dev-vm/health` を上書き（`STATE=OK|WARN|OFF`〔`OFF`＝QEMU プロセス無し〕・`CPU=<%>`・`CEIL=<%>`・`TS=<epoch>`・`MSG=…`）。`vm status`（§7）と orchestrator（§下）が読む。**鮮度**は `TS` で判定（読み手は「TS が古い＝監視停止」を非表示扱いにする）。
- **tmux バナー**: WARN の間 `tmux set -g @vm_health "⚠ VM資源逼迫…"`、OK 復帰で `tmux set -gu @vm_health`（クリア）。`tmux.conf` の status-right が `@vm_health` を表示（[30_scripts.md](30_scripts.md)）。OK→WARN 遷移時のみ `tmux display-message` でフラッシュ通知（`VM_HEALTH_COOLDOWN` 既定 600 秒で再フラッシュを抑制）。tmux サーバ未起動時（`tmux has-session` 失敗）はスキップ。**status-right のチップは桁数制約で短縮表記**だが、断定を避ける「可能性」の文言（08 §3.7）は health ファイルの `MSG`・フラッシュ通知・orchestrator バナーが担い、`vm status` での確認を促す。
...
- **orchestrator 連携**: 実行モードのダッシュボードが描画のたびに health ファイルを読み、`STATE=WARN` かつ `TS` が新しければ（鮮度の具体値＝既定 120 秒はダッシュボード側＝`dashboard.go` が持つ）画面上部へ警告バナーを出す（controller ループは非改変・`dashboard.go` に限定。正本 [60_orchestrator.md](60_orchestrator.md)）。
...
- [60_orchestrator.md](60_orchestrator.md): VM モード時に worker/ブレインストーミングプロンプト先頭へ `VM_DEV.md` ポインタを前置（発見導線2）。実行モードのダッシュボードが health ファイルを読み資源逼迫バナーを表示（§7.2）。
