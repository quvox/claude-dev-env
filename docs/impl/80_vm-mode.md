...
scripts/vm-up.sh (新規)           provision（初回）→ virtiofsd 起動 → QEMU 起動（常駐）→ ゲスト dockerd 準備を同期ポーリング → ポート自動同期(vm-portsync --loop)を常駐起動
scripts/vm (新規)                 コンテナ内ヘルパー（status/shell/restart/down/rebuild/portsync/logs）
