...
3. **cloud-init user-data** を `cloud-localds` で seed ISO 化。user-data は次を行う:
   ...
   - dockerd を **`tcp://127.0.0.1:2375`（ゲスト内ローカル）＋ unix socket** で待受（`/etc/docker/daemon.json` の `hosts` ＋ systemd drop-in の `-H` 調整）。
...
1. **virtiofsd 起動**（バックグラウンド）: `virtiofsd --socket-path=/run/vm/vfs.sock --shared-dir=/workspace`（共有タグ `workspace`）。
...
- Docker API（2375, 非TLS）は **hostfwd で `127.0.0.1` のみ**に露出＝claude コンテナ内からのみ到達。ネットワークには晒さない。
...
## 実装状況
本書は設計確定・**未実装**（次フェーズで実装）。現状はビルディングブロック（qemu 一式・`--kvm` デバイス渡し）のみ存在し、virtiofsd・provision・vm-up・`--vm`/`vm`・VM_DEV.md 生成は未実装（[08](../08_vm-mode.md) 参照）。
