...
| [11_cli-mac.md](11_cli-mac.md) | `claude-dev-mac` | ホスト側 CLI（macOS 版）の実装仕様。Linux 版との差分（SSH agent はプロジェクト単位の専用 agent＋socat TCP ブリッジ・鍵はローカル `.claude-dev.yaml` のみ・ssh-keys で対話選択・Docker ソケット検出・VM/KVM 拒否・ポート直結・ネイティブアーキ）。設計は [../09_macos-support.md](../09_macos-support.md)。 |
