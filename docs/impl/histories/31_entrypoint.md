...
- ステップ 14.5 を追加。非 VM かつ DOCKER_HOST が docker-proxy を指すとき dood-portsync.sh --loop を su $USERNAME で常駐起動（CLAUDE_DEV_DOOD_PORTSYNC=0 で無効）。DooD で 127.0.0.1:PORT を到達可能にする。
