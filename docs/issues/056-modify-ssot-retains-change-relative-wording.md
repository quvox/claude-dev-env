...
## 語彙の追加(2026-08-05 `/doc-check ssot task-relations-code-sync`)

**「旧実装」も同型の変更相対語であり、上の #1〜#23 の走査語彙に入っていなかった。**
反映後の SSOT に残る箇所は次の2件で、いずれも**本タスクが書き替えた節の外**にあるため直していない。

| # | 箇所 | 該当箇所 |
|---|---|---|
| 24 | `docs/02-design/system.md:303` | 「却下した案: 数字キーで即時移動する(**旧実装**)」 |
| 25 | `docs/03-impl/relations/MODULE-orchestrator-controller.md:256` | 実装上の判断1「…(**旧実装**の `runCancel()` による全停止をやめた)」 |
