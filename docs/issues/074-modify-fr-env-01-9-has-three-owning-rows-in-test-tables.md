...
- 2026-08-07 起票。`task-clause-ids-and-split-policy`(条項ID への移行)の `/doc-check` で、
  ...
  同タスクの決定シート 論点4 で人間が **A(今回は行を動かさず、02 の主担当を `MOD-cli-stop` に
  確定し、解消は本issueで追跡する)を選択した**(2026-08-07)。したがって本issueは
  `task-clause-ids-and-split-policy` の完了後も残り、別のタスクで解消する。
