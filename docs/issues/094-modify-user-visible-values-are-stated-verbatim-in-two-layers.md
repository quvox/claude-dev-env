...
related: FR-env-12-5, FR-orch-03-3, FR-env-01-18, FR-env-01-16, D0-dist-04, DSN-dist-02, CTR-cli-orchestrator, CTR-cli-container, docs/02-design/logging.md
pattern: 利用者が見る値(既定値・文字集合・鍵と値の組)が、上流の層と下流の層に逐語で独立に書かれている
pattern_survey: "`.claude/scripts/` に専用の検査は無いため、バッククォート語の層またぎ出現を機械で洗い出した(下の「同型の全件」に手順を記録)。候補 29 件のうち、値の重複に当たるのは 4 件・のべ 13 箇所である"
summary: 既定値・受理する文字集合・設定の鍵と値の組が 01 と 02(1件は 00 も)に逐語で書かれており、どちらを直せば正になるかが決まらない
---
