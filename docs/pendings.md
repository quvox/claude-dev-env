...
- 2026-08-19 削除済み issue を指す参照が**9件**残る(2026-08-20 の F3 が `046` の残り3件 — `03-impl/relations/MODULE-cli-list.md` / `MODULE-makefile-clean.md` / `MODULE-makefile-status.md` — を経緯の履歴 `docs/histories/2026-08-20-fix-session-list-undercount.md` を指す形へ直した。**冒頭の9件はその修正を反映した後の実測値である**)。`docs/issues/002` が2件(`03-impl/relations/MODULE-cli-common-write-project-ssh-keys.md:86` / `MODULE-cli-ssh-keys-reset.md:30`)、`docs/issues/092` が1件(`02-design/architecture.md:192`)— ここまでは `check-ssot.py` の CS11 が毎回報告する。`docs/issues/024` が6件(`pendings.md:88`・`:106` / `reverification.md:87` / `issues/028-modify-name-uniqueness-does-not-satisfy-nfr-scale-01.md:21`・`:58`・`:68`。**CS11 は 00〜03 層しか走査しないので、この6件はどの機械検査にも現れない**)。`002` / `092` / `024` は経緯の履歴が複数あって指す先が一意に決まらないため機械的には直せない。読者が根拠を辿れない。次に同ファイルを触るタスクが同じ降下で直す。
...
- 2026-08-20 `docs/03-impl/index.md:41`・`:48`・`:50` の検証記録が、2026-08-20 の規約刷新で廃止された `compose-changeset.py` / `check-changeset.py` / `.claude/directions/change-set.md` を指したままである。**日付で固定された検証記録**なので書き換えると当時の事実でなくなる(当時その道具で確認したこと自体は本当である)。道具名に「(現在は廃止)」を添えるか記録をそのまま残すかを、次に `03-impl/index.md` を触るタスクが決める(02 側の現在形の記述 — `docs/02-design/system.md` の HTML コメント — は 2026-08-20 の構築記録 `fix-fr-env-07-13-owner-and-codex-sandbox-defaults` が削除した)。
