...
- 2026-08-19 **`docs/03-impl/relations/MODULE-cli-start.md`:程度語が2箇所ある**: `:32` の「同一プロジェクトの2つ目の `start` が**数分**のイメージビルドを終えてから」と `:266` の「ビルドが走る(既存なら**即座に**完了する)」に、下限・上限も測定条件も無い(CLAUDE.md §7 の禁止語。`check-changeset.py` の CS8 の語彙には無いため機械検査には掛からない。独立レビュー Codex が検出)。どちらもコードの引用でも改訂記録でもない。次に同ファイルの当該節を触るタスクが、数値を入れるか語を落とすかを決める。
...
- 2026-08-12 **残る issue 10 件のうち 5 件に `origin_layer` が無く、6 件に `closes_when` が無い**(`005` / `010` / `028` / `046` / `055` は両方欠落、`094` は `closes_when` のみ欠落)。`.claude/directions/issues-pendings.md` §3 はどちらも必須としており、`check-ssot.py` の CS20 が 5 件を報告し続ける。いずれも `task-issue-sweep` より前から在る欠落で、それぞれの issue を次に扱うタスクが埋める(独立レビュー Codex が検出。2026-08-20 に `076` / `079` / `081` の削除を反映して件数を取り直した)。
- 2026-08-19 `docs/pendings.md` ほか10ファイルの参照節: 削除済み issue を指す参照が**9件**残る(**2026-08-20 の `fix-session-list-undercount` が closure 内の2件 —`02-design/contracts/cli-container.md` と `03-impl/index.md` — を経緯の履歴を指す形へ直した**) — `docs/issues/002` が3件(`02-design/architecture.md:192` は `092`、`MODULE-cli-common-write-project-ssh-keys.md:86` / `MODULE-cli-ssh-keys-reset.md:30`。`check-ssot.py` の CS11 が毎回報告する)と `docs/issues/024` が6件(`pendings.md:88`・`:106` / `reverification.md:87` / `issues/028-modify-name-uniqueness-does-not-satisfy-nfr-scale-01.md:21`・`:58`・`:68`。**CS11 は 00〜03 層しか走査しないので、この6件はどの機械検査にも現れない**)。読者が根拠を辿れない。次に同ファイルを触るタスクが同じ降下で直す。
