...
- 2026-08-19 `docs/pendings.md` ほか10ファイルの参照節: 削除済み issue を指す参照が11件残る — `docs/issues/002` が5件(`02-design/architecture.md:192` は `092`、`02-design/contracts/cli-container.md:141` / `03-impl/index.md:28` / `MODULE-cli-common-write-project-ssh-keys.md:86` / `MODULE-cli-ssh-keys-reset.md:30`。`check-changeset.py --ssot` の CS11 が毎回報告する)と `docs/issues/024` が6件(`pendings.md:88`・`:106` / `reverification.md:87` / `issues/028-modify-name-uniqueness-does-not-satisfy-nfr-scale-01.md:21`・`:58`・`:68`。**CS11 は 00〜03 層しか走査しないので、この6件はどの機械検査にも現れない**)。読者が根拠を辿れない。次に同ファイルを触るタスクが同じ降下で直す。
...
- 2026-08-19 `docs/03-impl/tests/e2e.md` の「実機確認の手順」節: E2E-01 手順7-3 の「出力に `SSH 鍵が未設定` の行が現れたら、**すぐに**同名の代役コンテナを立てる」に観測可能な同期点も上限時間も無く、競合の窓に入れるかが実行機の速度に依存する。`check-changeset.py` の CS8 は「すぐに」を曖昧語に数えない(独立レビュー Codex が検出。重大度「低」)。
...
- 2026-08-19 **`docs/02-design/system.md`「モジュール分割定義」と `docs/02-design/contracts/cli-container.md`「対応要件」が `FR-env-14` を欠く**: `system.md:52` の `MOD-entrypoint` 行の対応要件は `FR-env-02, FR-env-03, FR-env-05〜08, FR-env-11, FR-env-12` までで、同ファイルの要件カバレッジ表が `FR-env-14-11` の主担当を `MOD-entrypoint` としたことに追随していない。`02-design/contracts/cli-container.md:25` の対応要件も同様で、同契約は「渡す環境変数」表に `FR-env-14-6`〜`-10` の振る舞いを既に書いている。`task-fix-tmux-server-drops-reserved-env` は 03 側(`MODULE-entrypoint-claude` の `requirements`)だけを揃えた — 02 側の2節は変更指示に無く、足すには 10,782 バイトの節をまるごと置き換えることになるため見送った。次に同2節を触るタスクが同じ降下で直す。
