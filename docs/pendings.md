...
- 2026-08-19 `docs/pendings.md` ほか10ファイルの参照節: 削除済み issue を指す参照が**9件**残る(**2026-08-20 の `fix-session-list-undercount` が closure 内の2件 —`02-design/contracts/cli-container.md` と `03-impl/index.md` — を経緯の履歴を指す形へ直した**) — `docs/issues/002` が3件(`02-design/architecture.md:192` は `092`、`MODULE-cli-common-write-project-ssh-keys.md:86` / `MODULE-cli-ssh-keys-reset.md:30`。`check-ssot.py` の CS11 が毎回報告する)と `docs/issues/046` の残りが3件(`MODULE-cli-list.md:85` / `MODULE-makefile-clean.md:79` / `MODULE-makefile-status.md:73`。**2026-08-20 の `fix-start-auxiliary-halts-and-tmux-runtime-env` が closure 内の1件 — `02-design/contracts/cli-container.md` — を履歴を指す形へ直した**)と `docs/issues/024` が6件(`pendings.md:88`・`:106` / `reverification.md:87` / `issues/028-modify-name-uniqueness-does-not-satisfy-nfr-scale-01.md:21`・`:58`・`:68`。**CS11 は 00〜03 層しか走査しないので、この6件はどの機械検査にも現れない**)。読者が根拠を辿れない。次に同ファイルを触るタスクが同じ降下で直す。
