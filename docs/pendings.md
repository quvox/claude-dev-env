...
- 2026-08-19 **既に版管理の追跡下にある env ファイルを検出できない**: `docs/00-requests/decisions/sec.md` の `D0-sec-11` 項2 は「本システムは指定された env ファイルを版管理の追跡から外す」と述べ、項3 のガードレールは「既に追跡下に入ってしまったファイルまでは戻さない」と射程を限っている。手段は `.gitignore` への追記だけ(`MODULE-cli-start` 手順5-2 の 8)なので、**既に追跡されている env ファイルでは追記が成功しても追跡は外れず、`FR-env-14-4` の「外せなかったことを表示する」経路にも入らない** — 利用者は守られていると誤解する。`AC-08` の不合格条件「秘密の値がリポジトリへ入る」に当たる状態を、本システムは検出も表示もしない。仕様(00 項3)がそう決めているのでバグではなく、どの受入基準も塞いでいない。次に `FR-env-14` を要件側から動かすタスクが、検出して警告するかを判断する(独立レビュー Codex が `task-stop-cleanup-and-project-env` の `/doc-check` で検出)。
- 2026-08-19 **実装前の relations 変更指示が、まだ存在しないテスト識別子を `tests:` に列挙している**: `new-features/03-impl/relations/MODULE-docker-proxy-serve.md` の `tests:` が `TestLabelVolumeCreate_*` ほか4本を挙げるが、`docker-proxy/` にそのテストはまだ無い。同じタスクは `03-impl/tests/docker-proxy.md` を「実装前に `実装済み` の行を足すと事実と違う」として変更なしに落としており、**同じ理由が frontmatter の `tests:` にも効くはずなのに、そちらだけ作業指示として先に書いている**。`.claude/directions/03-impl.md`「★ Before the implementation exists」は relations の `tests:` について何も述べていない。キットは CLAUDE.md §3 により製品 DoD 未達の間は凍結なので、実施はリリース後(独立レビュー Codex が検出)。
