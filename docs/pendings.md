...
- 2026-08-10 `docs/issues/` の `related` の陳腐化(009 / 030 以外):`004` が `MODULE-orchestrator-controller` / `MODULE-orchestrator-state-io` / `CTR-cli-orchestrator` / `CTR-orchestrator-prompt` を、`094` が `FR-orch-03-3` / `CTR-cli-orchestrator` を、いずれも実在しない ID として指したままである。`095` は `pattern_survey` の実測値が「24 箇所」だが、`check-changeset.py --ssot` の現在値は 6 箇所である。4 件まとめて棚卸しする。
