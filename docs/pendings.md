...
- 2026-08-19 `docs/pendings.md` ほか10ファイルの参照節: 削除済み issue を指す参照が11件残る — `docs/issues/002` が5件(`02-design/architecture.md:192` は `092`、`02-design/contracts/cli-container.md:141` / `03-impl/index.md:28` / `MODULE-cli-common-write-project-ssh-keys.md:86` / `MODULE-cli-ssh-keys-reset.md:30`。`check-ssot.py` の CS11 が毎回報告する)と `docs/issues/024` が6件(`pendings.md:88`・`:106` / `reverification.md:87` / `issues/028-modify-name-uniqueness-does-not-satisfy-nfr-scale-01.md:21`・`:58`・`:68`。**CS11 は 00〜03 層しか走査しないので、この6件はどの機械検査にも現れない**)。読者が根拠を辿れない。次に同ファイルを触るタスクが同じ降下で直す。
