...
- 2026-08-19 **`docs/03-impl/relations/MODULE-cli-start.md`:程度語が2箇所ある**: `:32` の「同一プロジェクトの2つ目の `start` が**数分**のイメージビルドを終えてから」と `:264` の「ビルドが走る(既存なら**即座に**完了する)」に、下限・上限も測定条件も無い(CLAUDE.md §7 の禁止語。`check-changeset.py` の CS8 の語彙には無いため機械検査には掛からない。独立レビュー Codex が検出)。どちらもコードの引用でも改訂記録でもない。次に同ファイルの当該節を触るタスクが、数値を入れるか語を落とすかを決める。
