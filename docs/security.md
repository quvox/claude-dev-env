...
認証情報は `claude-dev-auth` ボリュームに保存され、`~/.claude-shared/` にマウントされる。コンテナ起動時に entrypoint が認証ファイルのみを `~/.claude/`（コンテナローカル）にコピーする。

```
claude-dev-auth ボリューム
    │
    ├── login 時: ~/.claude-shared/ にマウント → Claude Code が書き込み → 終了時にコピー
    │
    └── start 時: ~/.claude-shared/ にマウント (RW)
         └── entrypoint が認証ファイルを ~/.claude/ にコピー
              + ~/.claude.json への symlink を作成
              + 30秒ごとに変更を ~/.claude-shared/ に書き戻し
```
