...
**Chrome/VNC コンテナにマウントされるもの:**
- 認証情報ボリューム `claude-dev-auth` → `~/.claude/`（読み書き、Chrome 認証情報の共有用）
...
認証情報は `claude-dev-auth` ボリュームに保存され、`~/.claude/` に直接マウントされる。

```
claude-dev-auth ボリューム
    │
    ├── login 時: ~/.claude/ にマウント → Claude Code が直接書き込み
    │
    └── start 時: ~/.claude/ にマウント (RW)
         └── entrypoint が ~/.claude.json への symlink を作成
              → Claude Code が認証に使用
```
...
### 4. Chrome/VNC コンテナの分離

Chrome/VNC は専用の共有コンテナ（`claude-dev-chrome`）で動作する。Claude コンテナからは分離されており:

- Claude コンテナには Google Chrome がインストールされない（Playwright Chromium のみ、ヘッドレステスト用）
- Chrome/VNC コンテナにはプロジェクトディレクトリがマウントされない
- Chrome/VNC コンテナは `claude-dev-auth` ボリュームのみマウント（Chrome の認証情報共有用）
- noVNC ポート（6080）は全コンテナで共有される固定ポート
- CDP ポートは Chrome 内部で 127.0.0.1:9222 にのみバインドされる（Chrome 128+ のセキュリティ強制）。socat 二段リレー（Chrome 側: 0.0.0.0:9223 → 127.0.0.1:9222、Claude 側: 127.0.0.1:9222 → claude-dev-chrome:9223）で接続する。リレーポート 9223 は `claude-dev-net` 内でのみアクセス可能（ホストには公開しない）
...
| エンドポイント | 理由 |
|---------------|------|
| `POST /containers/{id}/attach` (WebSocket upgrade) | 直接ストリーム接続はプロキシを迂回する可能性 |
| `POST /swarm/*` | Swarm 操作はホストクラスタに影響 |
| `POST /plugins/*` | プラグイン操作はホストに影響 |
| `POST /configs/*`, `POST /secrets/*` | Swarm の秘密情報操作 |
