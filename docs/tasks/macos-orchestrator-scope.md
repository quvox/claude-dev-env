...
| # | 未決点 | 帰着 | 状態 |
|---|---|---|---|
| 1 | 上表の A / B / C のどれを採るか | **2026-07-31 の人間判断で closure。「オーケストレーターは後で開発するので放置する」＝ 当面 C（ギャップを明示保持）、将来 B（実装を要件に合わせる）** | closure 済み |
| 2 | 着手時期・macOS 実機の確保 | 未定（オーケストレーターの開発を再開するときに本作業ごと見直す） | 保留 |

> **本作業は着手待ちではなく「保留」である。**`cli-mac.md` が合格証を持たない状態は
> 意図的なもので、`/doc-check` で不合格が出ても本作業を思い出すための印として扱う
> （放置の判断そのものが記録されているので、毎回調べ直す必要はない）。
...
- 2026-07-31: `/implement` の後始末中に独立監査が検出。`cli-mac.md` を実装に合わせて同期した時点で
  要件との齟齬が顕在化したため起票。
- 2026-07-31: 人間判断「オーケストレーターは後で開発するので放置する」により **保留**。
  当面はギャップを明示保持し、`cli-mac.md` は合格証なしのまま運用する。
  **オーケストレーターの開発を再開するときに本作業から着手する（`/change` でフェーズ1）。**
