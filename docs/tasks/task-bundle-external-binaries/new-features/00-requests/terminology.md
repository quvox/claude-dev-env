...
sections:
  ...
  - "## 判断に迷いやすい区別"
deletes: []
reason: '`RQ-dist-02` / `D0-dist-05` / `FR-env-13` が使う新しい語「同梱外部バイナリ」を用語集に足す。決定シート 概念1 で人間が承認した外延をそのまま書く — 含む = 保守者がビルド前に用意しコンテナ内で利用者が直接実行する単一実行ファイル、含まない = エージェント CLI(版の解決とピン留めを持つ別物で `D0-dist-03` / `D0-dist-04` が支配する)・コンテナ内で使う同梱スクリプト(外部由来ではない)・設定/データファイル(実行するものではない)。「含む例・含まない例」は新設する語に必須である(`.claude/directions/00-requests.md`)。あわせて「判断に迷いやすい区別」へ「同梱外部バイナリ ⇄ 同梱エージェント CLI」の行を足す — 両者はどちらも「イメージに焼く実行ファイル」であり、版の決まり方と支配する決定が違うことを取り違えると `D0-dist-03` のバージョン解決の仕組みを外部バイナリにも要求する読み違いが起きる。既存の行は1行も変えない'
---
