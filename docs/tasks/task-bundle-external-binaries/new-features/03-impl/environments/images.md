...
sections:
  ...
  - "## 落とし穴"
anchors:
  - { section: "## 同梱外部バイナリの設置(externals/)", after: "## 何をどう制御しているか" }
deletes: []
reason: '`FR-env-13`(同梱外部バイナリ)の実装仕様を書く。この層が **`externals/` の規約の唯一の正**である(00 の `D0-dist-05` はディレクトリを名指すに留め、規約と設置先をここへ委ねている)。設置の仕組みは独立した節「同梱外部バイナリの設置(externals/)」として新設し、挿入位置を `anchors` で宣言する(親節の本文に子見出しを足す形は取らない — `.claude/directions/change-set.md` §2)。他の4節は既存の内容へ足すだけである。(1)「何をどう制御しているか」のステージ説明に、終端ステージが載せるものとして同梱外部バイナリを加える。(2) 新設節が対象の選び方(直下 = 全アーキ共通、アーキテクチャ名のサブディレクトリ = そのアーキのみ、`README.md` は除外)、設置先 `/usr/local/bin`、権限 0755・所有者 root、置く位置(両方の配布ステージの**最終**レイヤー。`SR-24` / `DSN-dist-01`)を持つ。**アーキテクチャの判定は `dpkg --print-architecture` で行う** — この Dockerfile が既に5箇所(apt のリポジトリ定義2件・Go・Terraform・GUI ブラウザの分岐 各1件)で使っている値で、`amd64` / `arm64` という語彙も一致し、BuildKit の自動 ARG に依存しない(レガシービルダーでも成立する)。(3)「関係するファイル」へ `externals/` と `externals/README.md` を足し、`Makefile` と CI ワークフローが変更不要である理由を書き添える。(4)「使い方」へ同梱物を足す/差し替える手順を足す。(5)「落とし穴」へ5件足す(うち1件はフェーズ3の実測で分かった `COPY` 層に全内容が残る件)。標準委任で決めた4点をこの層へ開示する(DS-04 の権限、DS-05 の設置位置2件、DS-06 のアーキテクチャ判定手段)'
---
