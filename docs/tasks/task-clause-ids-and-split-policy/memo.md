...
**回答済み: sheet.md(転記済み)**
(フェーズ1分の8件は memo-1.md。フェーズ2で追記した論点4・論点5 は 2026-08-07 に
人間が**どちらも A** で回答し、変更指示へ反映済み — 下の表。
...
フェーズ2で追記した2件は 2026-08-07 に回答を得た。

| # | 論点 | 結果 | 反映先 |
|---|---|---|---|
| 論点4 | `FR-env-01-9` のテスト対応行が3行に重複 | **回答=A** | 変更指示は変えない(行を動かさない)。02 の主担当は `MOD-cli-stop` で確定。重複の解消は `docs/issues/074` が追跡し、別タスクで扱う |
| 論点5 | `不可分` の要件が `部分` の条項を持つ | **回答=A** | `FR-env-01` と `FR-env-07` を `段階可(理由)` にした。理由欄で段階的な条項(`FR-env-01-19` / `FR-env-07-5`)を名指し、他の条項は不可分と明記。記録は `D1-split-01`(段階可は5件へ)と `D1-split-02`(理由欄の条項集合を変える判断も委任範囲外) |

## 未決点

**すべて帰着済み(2026-08-07)。未決点ゼロ。**

| # | 未決点 | 帰着 |
|---|---|---|
| 1 | `FR-env-01-9` のテスト対応行が2ファイル計3行に重複(cli-stop 1行 / cli-logout 2行。内容は異なる: logout側=E2E-01 手順8-5、reset側=手順8-12)。02 の主担当は1つしか書けない | **人間判断 → 解決**(論点4 = A。02 は `MOD-cli-stop`、重複は `docs/issues/074` が追跡) |
| 2 | **`FR-env-01` と `FR-env-07` は `分割可否: 不可分` なのに、条項 `FR-env-01-19` / `FR-env-07-5` の充足が `部分(P-005)`**。規範は「不可分の要件に部分の条項があれば例外なく PASS をブロックする」と定めるため、このままでは `/task-close` で 01・02 を検証済みにできない | **人間判断 → 解決**(論点5 = A。`FR-env-01` / `FR-env-07` を `段階可(理由)` にし、段階的な条項を理由欄で名指した。`D1-split-01` を5件構成へ更新) |

ドライラン(自走 + 独立レビュー2本)で出た未決点は上の2件だけで、どちらも人間の回答で帰着した。
**未決点ゼロ**なので `/implement` の入場条件(原則7)を満たす。
...
- 2026-08-07 **`/doc-check(task) 判定: PASS`(2回目。論点4・5 の回答を反映した後)**。
  レビュー: **サブエージェント**(修正箇所に絞った再監査。model=sonnet)。指摘4件を裁定 —
  重大度「高」1件(`D1-split-02` が `D1-split-01` の古い件数「3件」を参照していた)は修正し、
  件数を書き写さず `D1-split-01` を参照する形へ改めた(取り残しの再発を構造的に防ぐ)。
  「中」1件(`段階可` × `部分` の組み合わせ)は**規範どおり PASS をブロックしないが消えない指摘**
  として下に記録する。「中」1件(段階可の理由が「以前との比較」に留まる)と「低」1件(委任範囲と
  制約で除外規定が重複)は修正した。
  PASS 時点の変更指示35ファイルのハッシュ集約 `sha1sum` = `6ce0c8e78253c3fd`
  (closure の版は1回目の記録から動いていない)。
- **PASS をブロックしないが残る指摘(重大度「中」。`/task-close` でも消えない)**:
  `FR-env-01` / `FR-env-07` は `段階可` で、条項 `FR-env-01-19` / `FR-env-07-5` の充足が
  `部分(P-005)` である。これは妥当な `段階可 × 部分` であり、規範は「重大度『中』に留まり、
  PASS をブロックしないが消えもしない。**決して『高』へ上げてはならない**」と定める
  (`/doc-check` 認証段階 §3。不変則3 の例外は1つだけという理由)。
  解消は `docs/pendings.md` P-005 の「どうなったら解消が必要か」が満たされたとき。
- 2026-08-07 `/doc-check(task) 判定: 不合格(残存2件 — いずれも人間判断待ち。指摘の重大度「高」は0件)`(1回目)。
