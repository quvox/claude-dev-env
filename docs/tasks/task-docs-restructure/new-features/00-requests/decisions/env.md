...
sections: []
deletes: []
reason: 旧 `decisions.md` の開発環境まわりの決定(D-6/D-7/D-9/D-10/D-24/D-25)と要確認(D-23)を新 ID 体系へ移す。`03-impl/relations/` からは参照されていないが、`FR-env-*` の背景として必要。
---

<!-- change: add。ファイル全体の最終内容。ID は D0-env-nn。 -->
