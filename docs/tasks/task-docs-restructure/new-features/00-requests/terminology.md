...
sections: []
deletes: []
reason: 旧 `glossary.md` を新体系のファイル名と表構成(英語表記=識別子に使う語 を追加)へ移す。識別子の表記ゆれが 03 まで伝播するのを防ぐため、英語表記の列を新設する。
---

<!-- change: add なのでファイル全体の最終内容。旧 docs/00-requests/glossary.md は delete 指示で撤去する。
     用語の定義は変更しない。「英語表記(識別子)」列だけを新設した。 -->
