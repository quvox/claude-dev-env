...
sections: []
deletes: []
reason: 旧 `system.md` に同居していたアーキテクチャ・データモデル・主要フロー・設計判断を、新体系の `architecture.md` へ分離する(`system.md` はモジュール分割・テスト戦略・UI設計に専念する)。設計判断には `DSN-<area>-nn` を付与し、`03-impl/relations/` が参照している ID を実在させる。
---

<!-- change: add。ファイル全体の最終内容。
     ここに置く設計判断: DSN-arch-* / DSN-auth-* / DSN-dist-* / DSN-orch-*。
     モジュール分割の根拠(DSN-mod-*)・テスト方針(DSN-test-*)・UI(DSN-ui-*)は system.md が持つ。 -->
