...
sections: []
deletes: []
reason: 旧 `_steering/tech.md` が持っていた「テスト/lint コマンドの唯一の正」と「Codex実行設定」を、新体系で全スキルが参照する場所へ移す。コールグラフ抽出設定の節は新体系で必須のため新設する。
---

<!-- change: add。ファイル全体の最終内容。
     **ここに書かれたコマンド文字列が正**であり、全スキルがこの厳密な文字列を使う。
     見出し構造を変えないこと(スキルが見出し名で参照する)。 -->
