...
sections: []
deletes: []
reason: 新体系で必須の「設計が期待する連携」を新設する。旧 `system.md` は契約5件しか持たず、機能単位の連携の期待が存在しなかったため、02⇄03 の突き合わせ(取りこぼしの検出)が成立していなかった。
---

<!-- change: add。ファイル全体の最終内容。
     `03-impl/relations/` と対になる。PLAN-<slug> の slug は MODULE-<slug> と一致させる。 -->
