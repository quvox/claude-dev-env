...
deletes:
  ...
  - "### 分割の根拠"
reason: モジュール分割を CLI のサブコマンド単位(29モジュール)へ組み替える(決定シート 論点3・論点10)。アーキテクチャ・データモデル・主要フロー・設計判断は `architecture.md` へ、契約は `contracts/` へ分離する。
---

<!-- 反映後のファイルはテンプレート(.claude/templates/02-system.md)の見出し順に並べること:
     モジュール分割定義 → 分割の根拠 → 要件カバレッジ確認 → テスト戦略 → UI設計 → 未解決事項。
     旧「概要」「アーキテクチャ」「データモデル」「主要フロー」「エラーハンドリング方針」
     「設計判断と代替案」は architecture.md と各 contracts へ移設済みのため削除する。 -->
