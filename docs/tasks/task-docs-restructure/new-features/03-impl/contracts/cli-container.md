...
sections: []
deletes: []
reason: 02-design と同一 ID の契約を実装側にも置き、実際に渡している環境変数・マウント・起動オプションを定義箇所つきで記述する(新体系では 03 側の contracts/ が必須)。
---

<!-- change: add。フロントマターは /task-close が付与する
     (id: cli-container / kind: other / impl: claude-dev::main#start / source: docs/02-design/contracts/cli-container.md)。
     kind は other: 本システムに REST API は無く、この契約は起動時の環境変数・マウントの取り決めである。 -->
