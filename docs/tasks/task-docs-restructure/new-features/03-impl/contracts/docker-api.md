...
sections: []
deletes: []
reason: 02-design と同一 ID の契約を実装側にも置く。検査項目・判定順序・拒否理由は実装(`docker-proxy/main.go`)が唯一の事実であり、行番号つきで固定する必要がある。
---

<!-- change: add。id: docker-api / kind: other /
     impl: docker-proxy/main.go::validateContainerCreate / source: docs/02-design/contracts/docker-api.md

     kind を rest-api にしない理由: これは Docker Engine API の**透過中継**であり、本システムが
     定義する API ではない。契約の本体は「何を検査し、何を拒否し、何を書き換えるか」である。
     Docker Engine API 全体を OpenAPI として写し取ると二重管理になり必ずずれる(委任 D0-scope-01)。 -->
