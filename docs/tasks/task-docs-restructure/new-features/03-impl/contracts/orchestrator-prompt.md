...
sections: []
deletes: []
reason: 02-design と同一 ID の契約を実装側にも置く。プロンプトの組み立てと結果の解釈は品質ゲートの中核であり、実装シンボルつきで固定する。
---

<!-- change: add。id: orchestrator-prompt / kind: other /
     impl: orchestrator/mode.go::Mode.ResolveArgs / source: docs/02-design/contracts/orchestrator-prompt.md -->
