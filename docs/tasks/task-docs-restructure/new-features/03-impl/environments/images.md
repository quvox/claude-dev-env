...
sections: []
deletes: []
reason: 旧 `03-impl/devcontainer.md` が持っていた「イメージをどう作るか」を、モジュールではない領域として `environments/` へ移す(`DSN-mod-05`)。Dockerfile はコールグラフに入口を持たず機能表に載せられないため、この文書が唯一の記述になる。
---

<!-- change: add。id: images / source: docs/02-design/environments.md
     開発環境を「どう制御するか」= 仕組み。具体的な構成値(公開先・タグ)は infra/local/ が正。 -->
