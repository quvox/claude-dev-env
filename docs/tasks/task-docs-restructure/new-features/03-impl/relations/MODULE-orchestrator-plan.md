...
1. `ReadyTasks(plan)`: `Status == pending` かつ依存タスクがすべて `done` のものを返す。
   依存に `failed` / `blocked` を含むものは返さない。
2. `AllDone(plan)`: すべてのタスクが `done` なら真。
3. `AllSettled(plan)`: すべてのタスクが終端状態(`done` / `failed` / `blocked`)なら真。
   `waiting_human` が1件でも残っていれば偽。
4. `MarkBlockedByFailedDeps(plan)`: 依存に `failed` / `blocked` を含む `pending` タスクを
   `blocked`(run 内の終端)へ落とす。
5. `NormalizeForResume(plan)`: 中断時の状態を再開可能な形へ整える。
...
|---|---|---|
| 依存 ID が存在しないタスクを指す | その依存は満たされないものとして扱われ、タスクは ready にならない | 永久に着手されない(plan の不備) |
