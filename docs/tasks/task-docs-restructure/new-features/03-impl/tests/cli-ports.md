...
sections: []
deletes: []
reason: MOD-cli-ports に割り当てられた受入基準と機能間連携仕様書について、テストの有無を可視化する。新体系ではモジュールごとに1ファイルを持つ。
---

<!-- change: add。フロントマターは /task-close が付与する(id: cli-ports / scope: MOD-cli-ports)。
