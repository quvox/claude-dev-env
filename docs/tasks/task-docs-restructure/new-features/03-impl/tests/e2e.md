...
sections: []
deletes: []
reason: 旧 `03-impl/e2e.md` を新体系の対応表形式へ移す。E2E シナリオは 02 の一覧(E2E-01〜06)と1対1で、通過する機能を機能間連携仕様書の ID で示せるようにする。
---

<!-- change: add。ファイル全体の最終内容。
     02 の E2Eシナリオ一覧の全件に行を作る。一覧に無いシナリオのテストを作ってはならない。
     旧 ID(E2E-1〜E2E-6)は E2E-01〜E2E-06 へ再採番した。 -->
