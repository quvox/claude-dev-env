...
sections: []
deletes: []
reason: モジュール分割定義に含まれない領域(イメージのビルドと GHCR 配布。DSN-mod-05)に割り当てられた受入基準の行き場を作る。ここが無いと FR-env-09 の CI 側などがどのテスト対応表にも現れず、欠落が見えなくなる。
---

<!-- change: add。フロントマターは /task-close が付与する(id: images / scope: (モジュール外)イメージのビルドと GHCR 配布)。
