...
sections: []
deletes: []
reason: MOD-orchestrator に割り当てられた受入基準と機能間連携仕様書について、テストの有無を可視化する。新体系ではモジュールごとに1ファイルを持つ。
---

<!-- change: add。フロントマターは /task-close が付与する(id: orchestrator / scope: MOD-orchestrator)。
...
| 36 | MODULE-orchestrator-claude-exec — 機能全体 | claudebin.go に対応する単体テストが無く、E2E-4 の実機確認で代替する | 自動化の予定は無い(方針を変える場合は 02 の `DSN-test-01` から見直す) |
| 37 | MODULE-orchestrator-config — 機能全体 | config.go に対応する単体テストが無い | 自動化の予定は無い(方針を変える場合は 02 の `DSN-test-01` から見直す) |
| 38 | MODULE-orchestrator-main — 機能全体 | main.go に対応する単体テストは無く、E2E-4 / E2E-5 の実機確認で代替する | 自動化の予定は無い(方針を変える場合は 02 の `DSN-test-01` から見直す) |
