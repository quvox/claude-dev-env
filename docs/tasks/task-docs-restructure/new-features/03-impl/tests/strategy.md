...
sections: []
deletes: []
reason: 旧 `03-impl/e2e.md` は E2E シナリオしか持たず、レベル別の実行方法・絞り込み実行・データの後始末・状態語彙の定義が存在しなかった。新体系の `tests/` はこの1本を土台に、モジュールごとの対応表を持つ。
---

<!-- change: add。ファイル全体の最終内容。
     コマンドは docs/02-design/environments.md の厳密な文字列と一致させること。 -->
