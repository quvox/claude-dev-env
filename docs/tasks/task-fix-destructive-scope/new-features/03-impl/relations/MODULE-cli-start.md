...
sections:
  ...
  - "## 既知の制限"
deletes: []
reason: D0-env-08 の管理ラベルの付与側であり、項6 の排他(プロジェクト単位と、認証コピーから docker run 完了までの共有資源単位)と項7 の compose プロジェクト名の一意化を担う(FR-env-01 受入基準 14・16〜19。docs/issues/020 / 024 / 045) / 実装後に C-1 で 戻り値・副作用 / 異常系 / 既知の制限 を実装から確定させた(旧挙動の記述が残ると合成ビューが矛盾するため)
id: MODULE-cli-start
