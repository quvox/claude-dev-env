...
sections:
  ...
  - "## 既知の制限"
deletes: []
reason: D0-env-08 の対象限定と遊休判定のイメージ非依存化、および項6 の排他を stop へ降ろす(FR-env-01 受入基準 9・14〜17。docs/issues/045 / 024 / 020) / 実装後に C-1 で 戻り値・副作用 / 異常系 / 既知の制限 を実装から確定させた(旧挙動の記述が残ると合成ビューが矛盾するため)
id: MODULE-cli-stop
module: MOD-cli-stop
kind: tool
sync: sync
...
| 制限 | 影響 | 関連 issue |
|---|---|---|
| **本変更より前に起動した compose 資源は旧い名前(ハッシュ無し)を持つため片付けられない** | 移行期のあいだ、旧い compose 資源は手動で削除する必要がある(表示で気づける) | `docs/issues/024`(**根本は本変更で解消**。これは移行期の残り) |
