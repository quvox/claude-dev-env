...
sections:
  ...
  - "## 未検証(テスト未実装)の全件"
deletes: []
reason: FR-env-01 受入基準9 が logout と reset にも掛かるようになり(両方が遊休判定を通してから docker-proxy を消す。reset は claude-dev-net も)、中断時の終了コード 130(FR-env-03 受入基準23)が増え、FR-env-03 に受入基準 14〜22(確認・非対話時の中止・--yes・管理ラベルによる限定・削除失敗の列挙・対象0件・プロジェクト配下の認証コピーの削除・reset の非対称)が増えたため、対応表に行を足す。14〜18・22 は reset と共通または reset 側の振る舞いで、主担当としてこの表が持つ
---
