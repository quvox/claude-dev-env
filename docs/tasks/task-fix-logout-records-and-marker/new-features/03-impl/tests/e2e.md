...
sections:
  ...
  - "## テスト設計の判断"
deletes: []
reason: 'E2E-01 手順8 に本タスクの3件+1件の確認を足す。(1) **手順8-18 に (c) を新設** — 管理ラベル付きコンテナの集合を引けない状態で0件の経路に入らず、引けなかったことが「消えなかった資源」として列挙されて終了コード 1 になることを確かめる(`docs/issues/099`)。(2) **手順8-19 を新設** — `/auth` に印と同名のファイルを置いても「空」と判定しないことを確かめる(`docs/issues/100`)。**この手順は壊れていない環境でしか成立しない**ので手順8-18 の後片付けの後ろに置く。(3) **手順8-10 に追加** — `logout` が削除に失敗した実行でもラベル無しコンテナの名前と書き戻しの警告を出すことを確かめる(決定シート 論点1)。既存の失敗列挙の手順が同じ前提(共有ボリュームを使用中のコンテナを残す)を既に作っているので、独立した手順にせずそこへ載せる。**既存の手順番号は1つも動かさない**(`docs/pendings.md` P-006 と `03-impl/tests/cli-logout.md` が手順8-15・8-16 を名指しているため、追加はすべて末尾か既存手順の中に置く)。あわせて「テスト設計の判断」へ`[DS-01]` の行を1つ足す。**`docs/issues/098` の確認は手順8-18 の (a) と (b) の `--yes` 側が受ける**(消去を確認できない状態で削除まで到達させ、共有ボリュームが「削除した資源」に現れないことを見る)。(b) には**手順10 の列挙そのものが失敗する経路**の確認として `--yes` 側を新設した(独立レビューの指摘)'
---
...
8. **破壊的操作が自分が作った資源にだけ効くこと**(`FR-env-01` 受入基準 9・14〜27 /
   ...
   18. **共有ボリュームが空かを確かめられない状態では0件の経路に入らないこと**
      ...
      **このとき共有ボリュームが「削除した資源」の一覧に現れないこと**も確認する
      (`docs/issues/098` の確認点。消えたことを確認できていない資源を削除済みと表示しない)。
   ...
   19. **`/auth` に印と同名のファイルがあっても「空」と判定しないこと**
      (`FR-env-03` 受入基準19・18 / `docs/issues/100`)。**この手順は壊れていない環境で行う**
