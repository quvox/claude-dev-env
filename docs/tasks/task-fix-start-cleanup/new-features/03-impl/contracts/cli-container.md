...
sections:
  - "## 実装上の事実"
deletes: []
reason: FR-env-01 受入基準12・13(docs/issues/036)。「起動の再試行」の行が「失敗したら作りかけのコンテナを `docker rm -f` してから」と無条件に書いており、修正後の実装と食い違う
---

<!-- 変更指示。記法の正は .claude/directions/change-set.md
     ・変更は「起動の再試行」1行だけ。他の26行と節末の文章は現行のまま
       (replace は節ごと差し替わるので全行を書いている)
     ・`定義箇所` の行番号は修正でずれる。**/task-close がコードから再生成して確定させる**ので、
       ここでは現行の値を残している(実装後の実値が正) -->
