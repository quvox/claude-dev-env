---
target: docs/03-impl/relations/MODULE-entrypoint-claude.md
change: replace
version_bump: minor
reason: '`FR-env-07-13` と `FR-env-14-11`(コンテナへ渡した環境変数は、コンテナ内で起動したどのプロセスからも参照できること)を実現する側の仕様書。**変更点は5つ**: (1) **手順5 を「確定させる」へ** — macOS 経路では socat が作ったソケットで、ホストの `-e` では渡ってこないので entrypoint 自身の環境へも `export` する。(2) **手順6 の届く範囲を事実へ** — 初期化ファイルへの追記が効くのは対話シェルだけである。(3) **手順7 を書き直した** — 「rc への追記は非対話シェルに効かないから `-e` で渡す」という理由づけは、`-e` で渡した値が `su -l` を越えられないことを見落としていた。(4) **手順15 が VM の値を自身の環境へも `export`** する(対話シェルと tmux の窓で 2 つの値が同居しないように)。(5) **手順20 の `su` から `-l` を外した** — これが本体である。列挙して載せ直す形をやめたので、コンテナに渡っている変数がそのまま全部引き継がれ、利用者のプロジェクト環境ファイルの組も届く(`AC-08`)。`-l` の有無で `PATH` と `HOME` が変わらないことは実機で測って確かめた。**`[DS-05]` は 更新**(前の版の理由が実測で誤りだった。`delegation.md` §3.1)、**`[DS-02]` は 削除**(列挙をやめたので `set -e` の下で握る対象が消えた)。既知の制限を1行足し、異常系を1行入れ替えた。**既存の判断6件はすべて読み直し、いずれも継続と判断した。** `requirements` に `FR-env-14` を足す(02 のカバレッジ表が `FR-env-14-11` の主担当を `MOD-entrypoint` としたため。この層は要件単位の ID を並べる)'
...
20. **tmux セッションの開始**: `su "$USERNAME" -s /bin/zsh -c "cd /workspace &&
    ...
    **各ウィンドウの対話シェルは `exec zsh -l` で起こす**ので、対話シェル向けの初期化
    (手順5・手順6 が書き出した `export` を含む)はこれまでどおり効く。
