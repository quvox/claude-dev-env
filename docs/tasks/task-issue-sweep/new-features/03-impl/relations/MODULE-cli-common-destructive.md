---
target: docs/03-impl/relations/MODULE-cli-common-destructive.md
change: replace
version_bump: patch
reason: 'issue 051(CLI の利用者向け出力に生の Docker ID が混じる)を本タスクで解消し issue を削除するため、その issue を根拠として指す 2 箇所を、事実そのものの記述へ書き換える(**この機能は既に標準出力を捨てており正しい。残っていた漏れは `MODULE-cli-common-ensure-infrastructure` の `docker network create` 側である**)。**判断そのもの・理由・見直す条件は変えない**'
