...
change: replace
version_bump: minor
reason: 'issue 023 と issue 054 の 03 層。**(1) issue 023**: macOS の SSH agent 中継ポートはホストの環境変数から**無検証で**採られ(`claude-dev-mac:274`)、不正値でも socat の起動を試みたうえで**成功時と同じ表示**で起動が続くため、利用者は SSH 転送が効いていないことに気づけない(`D0-scope-07` の観測点の定義でいう「利用者が失敗に気づけない」に当たる)。`FR-env-04-8` の新設に合わせ、採用前に検証して不正値なら採用せず表示して続行する形へ改める。倒し方は `FR-env-04-5`(鍵ファイルが無ければ警告してスキップし続行)と同型で、`CTR-cli-container`「値の検証で起動を止めることはしない」に従う。**(2) issue 054**: 削除済みの `docs/issues/020`(4箇所)と `docs/issues/024`(1箇所)を `docs/histories/2026-08-04-fix-destructive-scope.md` へ付け替える'
id: MODULE-cli-start
updated: 2026-08-11
