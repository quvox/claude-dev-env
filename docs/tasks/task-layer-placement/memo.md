...
- [ ] `check-changeset.py` が変更指示に対して違反 0 件(**フェーズ3 でも再実行して `合格: 不変条件の違反なし`。
      チェックは付けない — この項目は「反映直前にもう一度実行する」ことを求めており、それは `/task-close` の工程である**)
- [x] `/doc-check task-layer-placement` が PASS(合成ビュー。未解決の重大度「高」が 0 件)— **2026-08-08 達成**
- [ ] 反映後に `check-changeset.py --ssot docs` を再実行し、**CS18 が 0 件**になっていること。
      他の検査(CS8 42 / CS11 19 / CS19 27 / CS20 53)が**凍結値から増えていない**こと。
      **`/doc-check`(2026-08-08)が合成ビューで先に実測済み**: CS18 0 / CS8 35 / CS11 19 / CS19 26 / CS20 53(NG 133)。
      反映後に `083` `085` `086` `090` `091` を削除すると CS20 はさらに 5 件減る
...
- [ ] 解消する issue の削除: `083` / `085` / `086` / `090` / `091`(**`093` は新規に起票したので残す**)
