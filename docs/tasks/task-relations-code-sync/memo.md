...
## 決定シート(フェーズ3末尾。2026-08-05 `/implement` が提示。**回答済み: #9 = A**)

**2026-08-05 に人間が「A でよい、close して」と回答した。** #9 = A なのでドキュメントの変更は無く、
QA レーンは対象外のまま `/task-close` へ進む。
...
- 2026-08-05 **フェーズ4(`/task-close`)**。§1 事前検査を通過(lint とテストを実際に再実行)。
  §2 で `build-callgraphs.py`「書き換えなし」/ `propose-features.py` FT2 **0件** を確認。
  §3 で**変更指示30件をすべて SSOT へ反映**(見出しの完全一致で解決。target 不在・重複 0 件)。
  反映中に `check-relations.py` の WARN 1件(controller の `callees` に足した slack に対応する
  本文の節が無い)をコードで裏取りして解消した。
- **`2026-08-05` /doc-check(ssot) 判定: PASS(1回目)。レンズ: Codex 6本中5本が成功**
  (`relations` 一括監査のみ 900 秒でタイムアウト)。合格証4件を発行。自動修正13件。
  **うち2件は反映の不具合**(変更指示の `sections:` に親子の見出しを併記したため子が二重挿入された)。
- **フェーズ4 §6 の決定シート(2026-08-05。人間が回答済み)**:

  | # | 論点 | 回答 | 適用結果 |
  |---|---|---|---|
  | 1 | `FR-orch-06` 受入基準2 を覆うテストが実在しないのに「実装済み」(`docs/issues/059`) | **B**(テストを書く) | **テスト本体は別タスク**(`/task-close` はコードを書けず、本タスクの「やらないこと」もコード変更を除外する)。SSOT には**いまの姿**を書いた: 当該行を `実装済み` → `未検証(テスト未実装)`、識別子を `-` へ。全件表に **#49** を追加し数え方を 47→48 行 / 表 48→49 行へ。裁定と解消条件は `docs/issues/059` に記録 |
  | 2 | `019`/`032`/`038` の削除で層00 の `decisions/orch.md:199` が実在しない issue を2件参照する | **A**(付け替える) | `D0-orch-15` の追跡先を `034`/`038` から **`058`/`059`** へ改めた。決定内容は不変なので **1.2.0 → 1.2.1(PATCH)**。再認証がこの文の対応のずれ(`059` は形式検証ではない)を指摘したため、**同じ合意の範囲内で**「形式の検証と採点基準の双方が…」へ言い直した(1.2.1 のまま) |
  | 3 | キット側: 入れ子見出しの二重挿入 / `callees` ⇄ 本文の節に 1:1 検査が無い | 既定 **C** | `.claude/improvements/KIT-nested-section-reflection-and-callee-body-check.md` に記録 |
  | 4 | relations 83本のうち独立にコード全文照合できたのは1本だけ | 既定 **A** | 3本目の memo へ申し送り(残り18本を1本1監査で回す案は `/doc-check full` に委ねる) |

- **`2026-08-05` /doc-check(ssot) 判定: PASS(2回目。§6 の回答適用後の再認証)。
  レンズ: なし(Codex がアカウント利用上限に到達。復旧は 2026-08-11 12:56)。**
  **不変則7 に従いサブエージェントによる代替は行っていない**(代替の可否は下のプロセス質問。
  既定 B =「代替しない」を適用した)。`ssot` 増分でありゲートではないため、
  `/codex-audit` の fail ポリシーどおり警告のうえ続行した。
  自動修正1件: **反映が持ち込んだ回帰** — 状態セルが `**未検証(テスト未実装)**` と太字になっており
  `build-index.py` の語彙判定から外れて索引が1件少なく出ていた(索引 47 ⇄ 本文の主張 48)。
  太字を外して `tests/orchestrator.md` を **1.4.0 → 1.4.1(PATCH)**、索引を **37 / 48 / 0** へ。
  **`decisions/orch.md` の合格証は失効していなかった**(PATCH は MAJOR.MINOR の比較で無視される)。

### プロセスに関する質問(不変則6・7。未決点ではないので原則7 のゲートを塞がない)

| # | 論点 | 選択肢 | 推奨案 | 適用した既定 |
|---|---|---|---|---|
| P4 | **2回目の認証で独立レンズが1本も立たなかった**(Codex がアカウント利用上限。2026-08-11 12:56 まで全リクエストが拒否される)。サブエージェントで代替するか | A 代替する / B 代替せず Claude 単独の検証で PASS を確定する / C Codex 復旧後にやり直す | **B**。対象は既認証文書への **PATCH 1件**で、その1件は機械的検査(語彙固定 ⇄ 索引の実測)で裏が取れている。代替は独立性を下げる(同系統モデルは盲点を共有する)割に得るものが小さい。**崩れる条件**: 以降 SSOT へ意味のある変更が入った場合 | **B(代替しない)**。1回目の認証では Codex 5本が実際に走っている |
