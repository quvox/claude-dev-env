---
target: docs/03-impl/relations/MODULE-orchestrator-dashboard.md
change: replace
sections:
  - "## 処理の流れ"
  - "## 呼び出され方"
...
<!-- 変更指示。反映後の最終形を書く。version / verified は持たない。frontmatter は `updated` の日付以外変更なし。
     ★docs/issues/032 #3 に対応して callers へ streamlog を足すことは**しない**:
     `dashboard.go::oneline` は features.md で「畳み込む薄いユーティリティ」と判定済みの
     共有関数であり、callees/callers はモジュール境界を跨ぐ呼び出しだけを載せる
     (.claude/directions/relations.md §3)。既知の制限にその判断が既に書かれている。 -->
