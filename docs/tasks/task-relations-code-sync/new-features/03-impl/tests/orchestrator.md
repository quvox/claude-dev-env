...
<!-- 変更指示。反映後の最終形を書く。version / verified は持たない。
     置換の根拠(2026-08-05 に `func Test` を機械抽出して照合):
     ・plan_test.go に TestReadyTasks_Basic は無く、TestReadyTasks_DependencyResolution /
       _ParallelLimit / _FailedDepExcluded の3つが実在する(機能単位の行はこの3つへ置換した)。
     ・FR-orch-05 受入基準2「未完了 plan の run 継続」は **「未検証(テスト未実装)」へ落とした**:
       「継続するか新規で始めるか」の分岐は orchestrator/main.go の再開判定にあり、そこに単体テストが
       1件も無い。近いテスト(archive_test.go::TestCountUndone / controller_test.go::
       TestResume_UsesResumeFlagAfterCrash)は「未完了数を数える」「同一 Attempt を --resume で継ぐ」を
       見ており、受入基準2 の主張そのものは覆っていない。**覆う範囲の判断なので質問キュー #1 で
       人間に確認する**(memo.md)。回答が「実装済み(上記2件)」なら、この行を
       `orchestrator/archive_test.go::TestCountUndone`,
       `orchestrator/controller_test.go::TestResume_UsesResumeFlagAfterCrash` / 実装済み に戻す。
       未検証行は不変則3 の例外により PASS をブロックしない。
     ・TestBuildQuestion_NumbersOptions は controller.go:1147 を、
       TestTerminalConfirm_NonTTYContinue は main.go:194 を検証している(term.go を呼ばない)。
     ・2026-08-05 /doc-check(task) の自動修正2件:
       (1) 「機能間連携仕様書 ⇄ テスト」の MODULE-orchestrator-plan 行が持つ 8 件に合わせて、
           relations 側の frontmatter(new-features/03-impl/relations/MODULE-orchestrator-plan.md)へ
           TestReadyTasks_ParallelLimit / _FailedDepExcluded を追加した(片側だけ 6 件で食い違っていた)。
       (2) 全件表 #38 の対象を「機能全体」から「フラグ解釈・ストア初期化・再開/新規の分岐」へ絞り、
           理由の「main.go に対応する単体テストは無く」を実態(terminalConfirm の1件だけある)へ直した。
           MODULE-orchestrator-main の ⇄ 表の行を「実装済み」へ変えたことと矛盾していたためである。 -->
