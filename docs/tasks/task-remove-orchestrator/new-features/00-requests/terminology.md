...
sections:
  - "# 用語集"
deletes: []
reason: 'オーケストレーターの全面削除にともなう用語の削除(決定シート 概念1)。用語表から 9 語を削除する: 「オーケストレーター」「コントローラ」「worker」「ブレインストーミングモード」「実行モード」「介入」「介入トリガー」「運用状態」「ORCHESTRATOR.md」。24 語 → 15 語になる。「判断に迷いやすい区別」からも 3 行を削除する: 「ブレインストーミングモード ⇄ 実行モード」「仕様(docs/) ⇄ 運用状態(.orchestrator/)」「Codex CLI の同梱 ⇄ 異種ベンダー worker の常用」(後者は `D0-orch-17` の未決を指しており、その決定事項ごと消える)。あわせて「資源逼迫」の定義から「ダッシュボードの表示は要件として課しておらず」という括弧書きを削除する — 参照先のダッシュボードが存在しなくなるため(`FR-env-08` 受入基準4 が課す tmux ステータス行の表示は変わらない)。`docs/issues/068`(「介入トリガー」の定義が発火経路を網羅していない)は語ごと消えるため解消して削除する。**`# 用語集` を親ごと差し替える**のは、用語表が `##` を持たず `# 用語集` の直下にあるためである(`.claude/directions/change-set.md` §2「親と子孫を同時に列挙しない」)'
...
| ブロック対象ドメイン | ファイアウォールが外向き通信を拒否するドメインの集合。**設定で与えられる**(**既定として同梱する集合の中身は 03 の実装仕様が持つ**)。**集合の中身は仕様で固定しない**(利用者が編集する前提のテンプレートである) | `pastebin.com`(ペーストサイト系)/ `webhook.site`(Webhook テスト系)/ `ngrok.io`(トンネリング系) | 適用されないコメント行として同梱されている雛形(集合に入らない)/ クラウドメタデータの IP(`169.254.169.254` ほか)と SMTP ポート(**別の規則**で拒否するのでこの集合に含まない) | `BLACKLIST_DOMAINS` | 「危険なサイト」「怪しいドメイン」(集合が特定できない表現) |
