...
change: replace
version_bump: patch
reason: 'オーケストレーターの全面削除にともなう `callers` と契機の修正(決定シート 概念1)。`MODULE-cli-orchestrate` を削除するので、`callers` が唯一の呼び出し元を失い「なし」になる。`## 呼び出され方` の契機からも、`orchestrate` が未起動時に `CLAUDE_DEV_NO_ATTACH=1` を付けて本機能を再帰的に呼ぶという記述と、macOS 版 `orchestrate` の未実装(`docs/issues/003`)への言及を削除する。**`CLAUDE_DEV_NO_ATTACH` そのものは環境変数として残す**(利用者が明示的に設定できる。実装の分岐は変えない)。この機能の振る舞い・処理の流れ・連携先・戻り値・異常系・実装上の判断・既知の制限は一切変えない。**`## 実装上の判断` を再掲する**: `.claude/directions/delegation.md` §3.1 に従って既存の判断行を1件ずつ読み直した結果、**すべて継続**である(オーケストレーターの削除で見直す条件が発火した行は1件も無い)。`CS19` はこの節が変更のたびに読み直されることを要求するので、変更が無い場合も再掲する'
id: MODULE-cli-start
