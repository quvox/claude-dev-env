...
change: replace
version_bump: patch
reason: 'オーケストレーターの全面削除にともなう委任 ID の付け替え(決定シート 概念1)。`## 実装上の判断` の判断3(「`resolveProjectDir` を変数として保持し、テストでスタブを注入できるようにする」)が根拠として `D0-orch-02` を引いているが、**`D0-orch-02` は削除する `docs/00-requests/decisions/orch.md` にしか存在しない**ため、このまま反映すると実在しない ID を指す(`CS11` 参照実在の違反になる)。付け替え先は **`D0-scope-02`(実装内部の構造上の判断 — ヘルパー関数の分割・統合、処理をどの経路に置くか)** とする。**そもそもこの引用は元から対象が合っていない**: `D0-orch-02` の委任範囲は「worker・対話 Claude の起動パラメータと指示の渡し方」で、テスト用のスタブ注入という論点を含まない。一方 `D0-scope-02` は「関数を変数として保持するかどうか」という実装内部の構造の選択そのものを覆い、ガードレール(利用者から観測できる振る舞いと契約を変えない)も満たしている(この判断は docker-proxy の応答を1つも変えない)。**判断の内容も、他の判断1〜11 も、本文の他の節も一字も変えない。**この付け替えは独立レビュー(readiness / lens: subagent)が重大度「高」として単独で検出したものである'
...
| 制限 | 影響 | 関連 issue |
|---|---|---|
| **所有者ラベルの注入に失敗しても、コンテナ作成経路ではログが1行も出ない** | 呼び出し元は特定できたのにボディを書き換えられなかった場合、片付け対象から外れた資源が生じたことを運用者が知る手段が無い(ネットワーク作成経路は理由を出す) | `docs/issues/087` |
