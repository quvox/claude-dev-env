...
sections:
  ...
  - "## テスト設計の判断"
deletes: []
reason: 'オーケストレーターの全面削除にともなうテスト実行方法の整理(決定シート 概念1)。**このファイルは実行される厳密なコマンド文字列を持つので、消えるモジュールを指したままにできない**。(1) `## レベル別の実行方法` から 単体(orchestrator)と 単体(自己検証題材)の 2 行を削除する。lint の備考「各 Go モジュールのディレクトリで」を `docker-proxy/` へ、結合の「上記2つの `go test`」を「上記の `go test`」へ、E2E の「実機操作 + 自己検証題材 / `make orch-sample` で題材を配置し `claude-dev orchestrate` で実走する / 手順による(E2E-04 は数十分)」を実機操作だけの形へ改める。(2) `## 絞り込んで実行する方法` から orchestrator と題材の 3 行を削除する(6 → 3 行)。(3) `## テストデータの準備と後始末` から 単体(Python)の行を削除し、結合・E2E(実機)の行から `make orch-sample` / `make orch-sample-clean` を外す。(4) `## フィクスチャ・ヘルパの流儀` から orchestrator の 3 行と自己検証題材の 1 行を削除する(6 → 2 行)。(5) `## 命名と配置の規約` から 単体(Python)の行を削除する。(6) `## カバレッジの扱い` の受入基準のカバレッジの現状を、機能要件 210 条項 → **140 条項**、対応表 225 行 → **152 行**(条項 150 件 = 140 + 非機能要件 10 件(`NFR-ops-05` の新設を含む)、加えて `FR-env-01-9` が主担当行を3つ持つ重複 2 行)へ改める。**重複 2 行を追跡する `docs/issues/074` はオーケストレーターと無関係なので残す**。(7) `## 状態列の語彙の定義` / `## 既知の不安定テスト` / `## テスト設計の判断` は変えない — いずれもオーケストレーターに依存しない'
---
