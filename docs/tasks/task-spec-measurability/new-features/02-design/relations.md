...
deletes: []
reason: >
  NFR-ops-01 の削除(決定シート概念#6)に伴い PLAN 5件の「対応要件」列から ID を外し、
  PLAN-makefile-update-claude の概要にある測定不能語「高速更新」を実装の事実へ置き換える
  (docs/issues/017。委任 b の範囲)。連携の構造は変えない。
---

<!-- 変更は2種類だけである。
     1. 「対応要件」列から `NFR-ops-01` を外す: `PLAN-container-tools-wait-limit-reset` /
        `PLAN-hooks-save-prompt` / `PLAN-hooks-send-slack-message` / `PLAN-vm-mode-cli` /
        `PLAN-vm-mode-healthd` の5件。いずれも他の FR が残るので根拠は消えない。
     2. `PLAN-makefile-update-claude` の概要:「Claude Code だけをキャッシュ利用で高速更新する」
        → 「コンテナイメージを作り直さずに Claude Code だけを更新する(ビルドキャッシュを使う)」。
        所要時間や比較対象を主張せず、`MODULE-makefile-update-claude` の実装が実際に行うこと
        (イメージの再作成を伴わない更新)だけを述べる。
     `PLAN-vm-mode-healthd` の概要にある「資源逼迫」は用語集の定義語なので書き替えない。 -->
