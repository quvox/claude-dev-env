...
deletes: []
reason: '想定連携の側を、片付けの射程(名前付きボリューム)とプロジェクトごとの環境変数へ合わせる。`PLAN-cli-stop` / `PLAN-cli-reset`: 削除対象の集合へボリュームを加え、**既定では消さず明示があるときにだけ消す**ことと、**削除の順序をコンテナ → ネットワーク・ボリュームで固定する**ことを書く。`PLAN-cli-stop` にはあわせて **存在しなかった資源を「削除できなかった」として扱わない**という期待を書く(`FR-env-01-34`。削除操作の終了コードでは存在しなかった場合と区別できない)。両者の対応する設計判断へ `DSN-arch-05`(遊休になった共有基盤は停止ではなく削除する)を加える。`PLAN-cli-start`: プロジェクト環境ファイルの読み取りを期待として書き、**この読み取りを共有基盤の機能にしない**判断(呼び出す側が `start` 1つだけで、共有基盤へ上げる条件を満たさない)を明記する。`PLAN-docker-proxy-serve`: 所有者ラベルを注入する要求へボリューム作成要求を加える。一覧の表では `PLAN-cli-start` の対応要件に `FR-env-14` を足す。**辺(呼び出し元・呼び出す先)は1本も増減していない** — 新しい機能を作らないためである。`PLAN-entrypoint-claude` / `PLAN-cli-common-*` の詳細と「未実装として認識しているもの」は変えない'
