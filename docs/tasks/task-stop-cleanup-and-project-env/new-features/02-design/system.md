...
sections:
  ...
  - "### E2Eシナリオ一覧"
deletes: []
reason: '4件を受ける。(1) **モジュール分割定義**: `MOD-cli-stop` / `MOD-cli-reset` の責務へ名前付きボリューム(削除の明示があるときだけ消す)を、`MOD-cli-start` の責務へプロジェクト環境ファイルの読み取りと `FR-env-14` を、`MOD-docker-proxy` の責務へボリューム作成要求への所有者ラベル注入を加える。**モジュールは1つも増やさない** — 環境ファイルの読み取りは `start` からしか呼ばれないので共有基盤の機能へ上げず、`DSN-mod-01`(モジュールは利用者から見た入口と1対1)の内側に収まる。(2) **要件カバレッジ確認**: `FR-env-01-28`〜`-34` と `FR-env-14-1`〜`-10` の 17 行を足し、条項数を 147 → 164 へ改める。主担当はそれぞれ1つで、`stop` 側が `MOD-cli-stop`、`reset` 側が `MOD-cli-reset`、環境変数が `MOD-cli-start` である。(3) **結合テスト対象**: `CTR-cli-container` の発行側の説明へボリュームを加える(責任モジュールと担当の割り当ては変えない)。(4) **E2E シナリオ一覧**: `E2E-01` へボリュームの扱い・存在しなかった資源の表示・プロジェクト環境ファイルの受け渡しを、`E2E-03` へボリュームの所有者ラベルを加える。**シナリオは1つも増やさない**(上流の `UC` が増えていないため)。**「分割の根拠」を親ごと `sections` に載せているのは `CS19` の要求による**(`docs/pendings.md` の 2026-08-11 の残務が記録している既知の制約)。**本文としては `DSN-mod-01`〜`DSN-mod-07` を1文字も変えていない** — 再読のうえ、いずれも継続と判断した(モジュールを増やさないため、機能数の目安にも影響しない)。「テスト戦略」のレベル別方針と「UI設計」「未解決事項」は変えない'
