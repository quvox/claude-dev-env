...
change: replace
version_bump: minor
reason: '**プロジェクトごとの環境変数の受け渡しを足す**(`RQ-env-07` / `D0-env-11` / `FR-env-14` / `DSN-env-05`)。手順5-2 として `load_project_env_file` を置き、`.claude-dev.yaml` の `env_file` が指すファイルを読んで `名前=値` の組を組み立て、手順13 の `PROJECT_ENV_OPTS` として **本システムが付与する変数より後ろに** `-e` で渡す。**読み取りは本機能に畳み込み、共有基盤の機能へ上げない**(呼び出す側が `start` 1つだけで、ファンイン2以上という昇格の条件を満たさない — `PLAN-cli-start` が同じ判断を持つ)。予約名・読めない行・重複・プロジェクト外の指定・ファイルが無い場合の倒し方をすべて「起動を止めない」に揃え(`FR-env-14-5`〜`-10`)、env ファイルを `.gitignore` へ追記する処理を既存の `.claude` / `.codex` の追記と同じ1つに載せる(`FR-env-14-4`)。異常系に6行、冪等性の表に1行、実装上の判断に3つの開示行を足す。**値そのものは表示もログもしない**(`FR-env-14-3`)。**呼び出し元・呼び出す先・契約は1つも増減していない** — 読み取りが本機能の内部に閉じるためである'
id: MODULE-cli-start
...
- 何のために呼ぶか: コンテナ内の初期化(UID/GID 追従・認証コピー・ファイアウォール適用・
  ...
  `claude-dev:1501` / `claude-dev-mac:1552`。手順15 の再試行ループの中にある)。
