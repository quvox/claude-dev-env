...
change: replace
version_bump: minor
reason: '所有者ラベルを注入する経路へ**ボリューム作成要求(`POST /volumes/create`)**を加える(`CTR-docker-api`「検査する要素と判定」/ `FR-env-07-11`)。`D0-env-05` 項2 の 2026-08-19 の改訂で名前付きボリュームが片付けの対象になり、**ボリュームを名前ではなく所有者ラベルで見分ける**と決まったため(`DSN-env-04`。名前で決めると別ディレクトリのデータを巻き込みうる)。**注入の本体(`injectOwnerLabels`)は変えない** — ボリューム作成要求のボディも `Labels` を最上位に持つので、既存の1本で足りる。**拒否判定は1つも増やさない**(ネットワーク作成と同じく、ボリューム作成にホストを危険に晒す要素は無い)。単体テストを4本足し、`tests:` に載せる。実装上の判断5・9・10 を3経路へ広げ、判断の開示を2行足す。既知の制限には**所有者ラベルの付かないボリュームは `--volumes` を指定しても消えない**ことを書く。**呼び出し元・呼び出す先・契約・要件は1つも増減していない**'
...
**あわせて、コンテナ作成要求・ネットワーク作成要求・ボリューム作成要求に所有者ラベルを付けて中継する**
...
3. リクエストごとに次の順で処理する:
   ...
   3. `POST` かつ `containerCreateRe` に一致 → `validateContainerCreate`。エラーなら `403`。
      **所有者ラベルの注入はこの関数の中で行う**(手順6)。ハンドラから別に呼ばないのは、
      ...
      置くのが最も短いからである。
   4. **`POST` かつ `networkCreateRe` / `volumeCreateRe` に一致 → 所有者ラベルを注入する**
      (手順6 と同じ注入を、拒否判定なしで行う専用の経路)。
      **拒否判定は持たない**(ネットワーク作成にもボリューム作成にも、ホストを危険に晒す要素は無い)。
      **ボリューム作成要求のボディも `Labels` を最上位に持つ**ので、注入は
      `injectOwnerLabels` の同じ1本で足りる(判断10)。
...
| 永続化 | なし。**触る資源は Docker Unix ソケット `/var/run/docker.sock`** と、インメモリの `ip → PROJECT_DIR` キャッシュ(TTL 60 秒・`sync.Mutex` 保護・永続化なし)。**副作用として、作成されるコンテナ・ネットワーク・ボリュームにラベル `claude-dev.role` / `claude-dev.owner-project-dir` が付く**(資源そのものは Docker が作る) |
| 発火するイベント | なし |
...
| 5 | **所有者ラベルの注入を、拒否判定をすべて通過したあとに置く。** 拒否される要求に印を付けても意味が無く、注入を先に置くと注入の失敗が拒否判定を飛ばしうる。**ネットワーク作成要求とボリューム作成要求は拒否判定を持たない**ので、注入だけを行う | D0-env-10 / `DSN-env-04` |
...
| 9 | **ネットワーク作成とボリューム作成のパスを、コンテナ作成と同じく版接頭辞を許す正規表現で判定する**(`/v1.45/networks/create` と `/networks/create`、`/v1.45/volumes/create` と `/volumes/create` の双方が来る) | D0-scope-02 |
| 10 | **注入の本体を `injectOwnerLabels(body, owner) ([]byte, bool)` として切り出し、コンテナ作成・ネットワーク作成・ボリューム作成の3経路から呼ぶ。** 書き戻しも `writeBackBody(r, body)` の1つに集約する。3経路で「トップレベルの `Labels` に2つ書く」ことは同一なので、分けると片方だけ直る経路ができる。**ボリューム作成を加えるにあたって注入の本体は変えていない**(ボディの形が同じであるため) | D0-scope-02 |
...
| **名前を指定せずに作られたボリューム(匿名ボリューム)には所有者ラベルが付かない** | その作成要求は発行されず、Docker がコンテナ作成の中で直接作るため、この機能は関与しない。**片付けは印に頼らず、紐づくコンテナの削除に添える指定で行う**(`CTR-cli-container` の規則 D / `MODULE-cli-stop` 手順8-1) | なし(閾値の外: **02 の契約が別の手段を定めている**) |
| **呼び出し元を特定できない要求で作られた資源には所有者ラベルが付かない** | その資源は `stop` / `reset` で片付けられず、ホストに残る。**名前付きボリュームでは `--volumes` を指定しても消えない。** **利用者への表示も行わない**(印が無い以上、存在を知る手段が無い) | なし(閾値の外: **00 が「そう決めた」と明示しているもの**。`D0-env-08` 項8 / `FR-env-07` 受入基準12) |
...
- [DS-05] **注入の呼び出し口を `labelCreateRequest(r, logger, kind)` の1本にし、`labelNetworkCreate` / `labelVolumeCreate` はそれを呼ぶだけの薄い入口にする** — 理由: 2経路が「所有者を引く → 注入する → 書き戻す → 結果をログへ出す」を完全に共有しており、違うのはログに出す資源の種別だけである。種別を引数で受けると、**どちらの正規表現に一致したかがそのまま種別になる**(パスから取り直さずに済む)。既存のテストが呼ぶ `labelNetworkCreate` の名前と引数は変えていない / 見直す条件: 経路ごとに注入の内容が変わるようになったとき
- [DS-05] **ボリューム作成の判定を `volumeCreateRe` として独立した正規表現で持ち、ネットワーク作成の判定とまとめない** — 理由: 1本の正規表現に交替(`networks|volumes`)を書くと、ログに出す資源の種別をパスから取り直すことになる。別々に持てば、どちらに一致したかがそのまま種別になる / 見直す条件: 注入対象の作成要求がさらに増えて、種別ごとの分岐が3つを超えたとき
- [DS-01] **ボリューム作成経路の単体テストを、ネットワーク作成経路と同じ4本の型で置く**(注入される / 所有者が空なら素通し / 版接頭辞つきのパスに一致する / 利用者が指定した同名ラベルを上書きする) — 理由: 注入の本体は共通なので、経路ごとに確かめるべきは「その経路が注入を呼ぶか」だけであり、既にある経路と同じ型にすると読み手が差分だけを見られる / 見直す条件: 経路ごとに注入の内容が変わるようになったとき
