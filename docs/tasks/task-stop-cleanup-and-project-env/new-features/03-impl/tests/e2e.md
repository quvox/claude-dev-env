...
sections:
  ...
  - "## テスト設計の判断"
deletes: []
reason: '`03-impl/tests/cli-stop.md` / `cli-reset.md` / `cli-start.md` が新しく参照する実機確認の部分手順を書く。**E2E-01 の末尾へ3つ足す**: 手順8-21(名前付きボリュームの片付け。`--volumes` 無しで残ること → 有りで消えること → 0件のとき表示しないこと → 起動元ディレクトリを読めないとき行わないこと。`FR-env-01-28`〜`-31`)、手順8-22(**存在しなかった資源を「削除できなかった」と表示しないこと**。`FR-env-01-34`。**compose 既定ネットワークを作らない構成でだけ再現する**ので前提としてその構成を指定する — 2026-08-18 の実測)、手順8-23(プロジェクトごとの環境変数の 10 条項。`FR-env-14`)。**E2E-03 にはボリューム作成要求への所有者ラベルの付与**を足す(`FR-env-07-11`)。**既存の部分手順の番号は1つも動かしていない** — 対応表と `docs/pendings.md` が外から参照しており、詰めると参照が解決したまま別の手順を指すためである(その判断をテスト設計の判断に開示した)。**`E2E-02` / `E2E-06`・「E2Eシナリオ ⇄ テスト対応表」・「通過する機能」・「テスト環境」・「未検証(テスト未実装)の全件」・「対象外としたシナリオ」は変えない**'
