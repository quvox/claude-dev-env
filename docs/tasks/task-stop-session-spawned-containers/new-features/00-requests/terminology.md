...
sections:
  - "# 用語集"
deletes: []
reason: '`D0-env-05` 項2 の拡張と `D0-env-08` 項8 の新設にともなう用語の整備。(1) 「管理ラベル」の定義が「本システムが Docker 資源を**作るとき**に付ける」となっており、docker-proxy が利用者の作る資源へ付ける所有者ラベル(`D0-env-08` 項8)を覆わないため、「本システムが作る、または本システム(docker-proxy)を通して作られる Docker 資源に付ける」へ改め、含む例・含まない例も足す。(2) 「セッション由来の資源」を新しい語として追加する(`FR-env-01` / `FR-env-07` / `CTR-cli-container` / `CTR-docker-api` が使う)。定義・含む例2件・含まない例・英語表記・使ってはいけない言い方を `.claude/directions/00-requests.md` の規則どおりに埋める。(3) 「破壊的操作」の含む例に `claude-dev stop` を足す(`D0-env-05` 項2 の拡張で、`stop` は利用者の作業内容を失わせうる削除を持つようになったため。定義文はもともと `stop` を含んでおり、例だけが欠けていた)。この節は表の親が `# 用語集` の1つだけなので、子節「判断に迷いやすい区別」も同じ本文に含めて最終形を書く(変更点は「セッション由来の資源」と「compose 資源」の対比を1行足すこと)。表の他の行は現状のまま変えない'
---
...
| セッション由来の資源 | ある Claude コンテナの中から **docker-proxy を経由して**作られた Docker 資源。対象は**コンテナとネットワーク**であり、`claude-dev stop` / `claude-dev reset` の片付け対象になる(`D0-env-05` 項2 / `D0-env-08` 項8) | コンテナ内で `docker run -d nginx` として作られたコンテナ / コンテナ内で `docker compose up` として作られたコンテナと compose 既定ネットワーク | **名前付きボリューム**(利用者のデータであり片付けの対象にしない。`D0-env-05` 項2)/ **VM モードのゲスト内で作られたコンテナ**(ホストの Docker に現れない)/ 利用者がホスト側で直接 `docker run` したコンテナ(docker-proxy を通らないので所有者ラベルが付かない)/ `fwd-*` 中継コンテナ(ホスト CLI が作る) | session-spawned resource | 「子コンテナ」「compose のコンテナ」(前者は親子関係の別概念、後者は経路を compose に限る言い方になる) |
...
| セッション由来の資源 | compose 資源 | 前者は**作られた経路(docker-proxy を通ったか)**で決まる集合で、`docker run` も compose も含む。後者は**compose が作ったもの**という部分集合で、`com.docker.compose.project` ラベルでも引ける。前者は所有者ラベル、後者は一意化した compose プロジェクト名で識別する(`D0-env-08` 項7・項8) |
