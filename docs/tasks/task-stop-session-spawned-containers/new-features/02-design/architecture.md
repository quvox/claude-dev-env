...
sections:
  ...
  - "## 設計判断"
deletes: []
reason: '`/doc-check`(2026-08-07)の独立レビューの指摘により影響範囲に加えた。当初は「4層構成・コンポーネントの責務・データの流れの分類は変わらない」として変更なしと判断したが、`DSN-env-04`(docker-proxy が所有者ラベルを注入する)の新設で2箇所が事実と合わなくなる。(1) 「コンポーネントの責務」の docker-proxy 行が「Docker API の検査・書き換え・拒否」だけを挙げており、**所有者ラベルの注入という新しい責務**を持たない。同じ 02 層の `system.md`「モジュール分割定義」の `MOD-docker-proxy` 行には注入を書いたので、書かないと 02 の中で2つの答えが残る。対応要件にも `FR-env-01`(片付ける側が読む印を出す責務)を足す。(2) 「データの流れ」の3(Docker 操作)が結果を「許可/書き換え/拒否」の3値で書いているが、改訂後の `CTR-docker-api`「通信の形」は「許可(透過)/ 書き換えて許可 / **所有者ラベルを付与して許可** / 拒否」の4値であり、検査の対象にもネットワーク作成要求が加わった。(3) **規範の更新(`.claude/directions/delegation.md` §3.1・検査 CS19)により「## 設計判断」を影響範囲に加えた** — この節を触る変更は既存の判断を全件読み直すことが要求される。8件(`DSN-arch-01`〜`04` / `DSN-auth-01` / `DSN-dist-01`・`02` / `DSN-orch-01`・`02`)を読み直した結果、**7件は継続**(文面を変えない)で、**`DSN-arch-02` の1行だけが更新**である: 状態の置き場の表が Docker リソースの所有を「ホスト CLI / Makefile」・識別を「`claude-dev-` 接頭辞で命名」としていたが、**セッション由来のコンテナとネットワークは docker-proxy が作成要求の中継時に印を付ける資源であり、名前は利用者が決めるので接頭辞の規則を課せない**。`DSN-env-04` が導入した所有者ラベルによる識別を、同じ表の別行として足す(既存行は1文字も変えない)。**4層構成・主要フロー・残る7件の設計判断は動かない**'
