...
| 8 | **ボディの再構成を1回にまとめる**(bind の書き換えと所有者ラベルの注入が同じ要求で起きうるため)。2回に分けると `Content-Length` の更新が二重になり、片方の書き換えが失われる経路ができる。**したがって手順5 は書き戻さず、手順6 の末尾で1回だけ `r.Body` / `ContentLength` / `Content-Length` を更新する**。**`docker-proxy` の既存の単体テストは全件(`cd docker-proxy && go test ./...`)、この分解の変更後も同じ判定結果を返すことを回帰として確かめる**(テストの分解が変わってもよいが、覆う条件を減らしてはならない。**本数を書かないのは、frontmatter の `tests:` の列挙件数とコード上のテスト関数の数が一致していないためである** — `docs/issues/082` が追跡する) | D0-scope-02 |
...
| **管理ラベルを持たない Claude コンテナからの要求にも所有者ラベルが付かない** | 管理ラベルが付く前に起動されたコンテナの中から作られた資源は片付け対象から外れる。**bind の書き換えは効くので、利用者から見て Docker は普通に使える**(気づく手掛かりが無い) | なし(閾値の外: **`stop` 側も同じラベルが読めないので片付けをスキップし、その旨を表示する**(`FR-env-01` 受入基準23)。表示にたどり着く経路が別に存在する) |
