...
sections:
  ...
  - "### E2E-03"
deletes: []
reason: '`FR-env-01` 受入基準22〜27 と `FR-env-07` 受入基準11 の実機確認手順を追加する。(1) E2E-01 の手順8 に、サブ手順14「セッション由来の資源が `stop` で消えること」と 15「`reset` が所有者を問わず消すこと」を挿入し、既存の後片付け(旧14)を 16 へ繰り下げる。手順14 では **`docker run` で作ったコンテナと `docker network create` で作ったネットワークの両方**を対象にし、削除した名前が種別つきで表示されること・0件のときに表示が出ないこと・**別セッションの資源が消えないこと**・**所有者ラベルを持たない資源が消えないこと**を確認する。手順16(後片付け)の対象にも追加分を足す。(2) E2E-03 に手順5「作成された資源に所有者ラベルが付いていること」と手順6「呼び出し元を特定できないときに拒否されないこと」を足す。(3) E2Eシナリオ ⇄ テスト対応表の E2E-01 / E2E-03 のシナリオ欄を、02 の `system.md` の同じ行と一致させる。(4) 通過する機能の E2E-01 に `MODULE-cli-reset`、E2E-03 に所有者ラベルの付与が `MODULE-docker-proxy-serve` を通ることを書き足す。**状態列はすべて `未検証(テスト未実装)` のまま**(シェル実装に自動テストランナーが無い方針は変わらない)。(5) **`/doc-check`(2026-08-07)の独立レビュー3本の指摘により3点を直した**: E2E-01 手順8-14 に**削除に失敗しても続行すること**を確かめる部分手順7 を足した(`03-impl/tests/cli-stop.md` は `FR-env-01-24` のテスト識別子を手順8-14 としていたが、手順8-14 には削除失敗を起こす部分手順が1つも無く、実際には検証しない手順を指していた)/ E2E-03 に**後片付けの手順7** を足した(新設の手順5 が `-d ... sleep 60` のコンテナ2つとネットワーク1つを作りっぱなしにするので、2回目に流すと同名で失敗する。従来の手順1〜4 は名前を付けない `docker run alpine true` だけだったのでこの問題が無かった)/ 対応表の E2E-03 のテスト識別子欄が**所有者ラベルの付与を「単体で検証済み」**と書いていたが、`03-impl/tests/docker-proxy.md` は `FR-env-07-11`・`12` を `未検証(テスト未実装)` としており同じ層の中で矛盾するため、追跡先を名指す書き方へ直した'
---
...
|---|---|---|---|---|
| E2E-01 | UC-01 | `claude-dev start`(ブラウザ確認あり / `--no-vnc`)→ `/workspace` マウント・認証・ファイアウォール・tmux → `claude` 起動 → 再実行での再接続 → **同名衝突で稼働中のコンテナを失わないこと(手順7)→ 破壊的操作が「自分が作った資源」にだけ効くこと(手順8: 管理ラベル・遊休判定・排他ロック・ラベル無しコンテナの保護・compose 資源の隔離・受理しない名前・プロジェクト配下の認証コピー・確認と非対話時の中止・削除失敗の列挙・**セッション由来の資源の片付け(手順8-14・8-15)**)** | 手順のみ(下記「実機確認の手順」E2E-01) | 未検証(テスト未実装) |
| E2E-02 | UC-02 | `claude-dev forward` → 8100 番台の割当と SSH トンネル → クライアントのブラウザで表示 → `claude-dev ports` で確認 | 手順のみ(同 E2E-02) | 未検証(テスト未実装) |
| E2E-03 | UC-03 | コンテナ内で危険な `docker run` → 拒否 / `/workspace` bind の許可 / 通常操作の透過 / **作られたコンテナとネットワークに所有者ラベルが付くこと** | 手順のみ(同 E2E-03)。判定ロジックは `cd docker-proxy && go test ./...` が単体で検証済み。**所有者ラベルの付与(`FR-env-07-11`・`12`)の単体テストは未実装で、`03-impl/tests/docker-proxy.md` の未検証の全件3・4 が追跡する** | 未検証(テスト未実装) |
...
8. **破壊的操作が自分が作った資源にだけ効くこと**(`FR-env-01` 受入基準 9・14〜27 /
   ...
   1. **管理ラベルの付与**(`FR-env-01` 受入基準14): `aaa` で
      ...
      `claude-dev.` で始まるラベルが**含まれない**ことも確認する(ラベルは Claude コンテナと
      セッション由来の資源にだけ付ける。docker-proxy は固定名で識別する)。
   ...
   8. **`logout` がプロジェクト配下の認証コピーを消すこと**(`FR-env-03` 受入基準 20・21):
      ...
      **(d)** 認証コピーが1つも無い状態で再度 `claude-dev logout` を実行すると、対象が無い旨を
      ...
      (`FR-env-03` 受入基準22。`logout` との非対称は意図したものである)。
   9. **確認と非対話時の中止**(`FR-env-03` 受入基準 14〜16): `claude-dev logout` で `n` を
      ...
      **何も削除せず終了コード 1** で終わり `--yes` の指定方法を表示すること、
      ...
      同じ3つを確認する(**`reset` は非 TTY で 0 ではなく 1 を返すのが正しい**。
      従来の `yes y | claude-dev reset` は `--yes` に置き換わる)。
   ...
   11. **`stop` を別ディレクトリから実行しても compose を取り違えないこと**(`FR-env-01` 受入基準 19・21):
      ...
      **`aaa` の compose コンテナが消える**ことを確認する(`claude-dev.project-dir` ラベルから
      ハッシュ源を得ているため、カレントディレクトリに依存しない)。
   ...
   16. 後片付け: `docker rm -f legacy-claude legacy-compose legacy2 nolabel nolabel2 nospawn spawn-a spawn-b spawn-a2 spawn-b2 spawn-b3 netholder` /
      ...
      `rm -rf ~/.claude-dev/locks` / 一時ディレクトリの削除。手順10・12・15 で `reset` を実行した場合は
      `claude-dev setup`(またはイメージの再取得)で環境を戻す。
   - macOS(`claude-dev-mac`)でも同じ手順を実行する。実行できない場合は
     **未実施であることを記録する**(手順を省いたことを黙って残さない)。
...
7. 後片付け: `docker rm -f label-check label-override` / `docker network rm label-net`。
   **手順5 が作るコンテナは `-d ... sleep 60` で名前を占有したまま停止済みとして残る**ので、
   消さずに2回目を流すと同名の `docker run` が失敗する(手順1〜4 の `docker run alpine true` は
   名前を付けないためこの問題を持たなかった)。
